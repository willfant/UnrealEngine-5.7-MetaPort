				...
				if (LooseParameterBuffer.BaseIndex == Parameter.GetBufferIndex())
				{
					...
					for (int32 LooseParameterIndex = 0; LooseParameterIndex < Parameters.Num(); LooseParameterIndex++)
					{
						...
						if (Parameter.GetBaseIndex() == LooseParameter.BaseIndex)
						{
							...
							// BEGIN META SECTION - Bound Uniform Local Lights
							ensureMsgf(!bValidateDataSize || DataSizeBytes == Parameter.GetNumBytes(), TEXT("Attempted to set fewer bytes than the shader required.  Setting %" SIZE_T_FMT " bytes on loose parameter at BaseIndex %u, Size %u.  This can cause GPU hangs, depending on usage."), DataSizeBytes, Parameter.GetBaseIndex(), Parameter.GetNumBytes());
