...
)
{
	...
	bool bUseClusteredReflections = MobileForwardEnableClusteredReflections(ShaderPlatform);
	bool bSkipLightGrid = bEnableNoLightGridIfNoLightOpt && NumLocalLights == 0 && (!bUseClusteredReflections || NumReflectionCaptures == 0);
	if (!bSkipLightGrid)
	{
	// END META SECTION - LightGrid NOLightGridIfNoLight
...
FComputeLightGridOutput FSceneRenderer::ComputeLightGrid(FRDGBuilder& GraphBuilder, bool bCullLightsToGrid, const FSortedLightSetSceneInfo& SortedLightSet, TArray<FForwardLightUniformParameters*, TInlineAllocator<2>>& PerViewForwardLightUniformParameters)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		{
			int32 LocalLightNum = ForwardLightData.Num();
			...
			{
				View.ForwardLightingResources.ForwardMovableLightData.LightPositionAndInvRadius[LightIndex] = ForwardLightData[LightIndex].LightPositionAndInvRadius;
				View.ForwardLightingResources.ForwardMovableLightData.LightColorAndIdAndFalloffExponent[LightIndex] = ForwardLightData[LightIndex].LightColorAndIdAndFalloffExponent;
				View.ForwardLightingResources.ForwardMovableLightData.LightDirectionAndSceneInfoExtraDataPacked[LightIndex] = ForwardLightData[LightIndex].LightDirectionAndSceneInfoExtraDataPacked;
				View.ForwardLightingResources.ForwardMovableLightData.SpotAnglesAndSourceRadiusPacked[LightIndex] = ForwardLightData[LightIndex].SpotAnglesAndSourceRadiusPacked;
				View.ForwardLightingResources.ForwardMovableLightData.LightTangentAndIESDataAndSpecularScale[LightIndex] = ForwardLightData[LightIndex].LightTangentAndIESDataAndSpecularScale;
				View.ForwardLightingResources.ForwardMovableLightData.RectDataAndVirtualShadowMapIdOrPrevLocalLightIndex[LightIndex] = ForwardLightData[LightIndex].RectDataAndVirtualShadowMapIdOrPrevLocalLightIndex;
			}
		}
		// END META SECTION - LightGrid LightDataUBO
