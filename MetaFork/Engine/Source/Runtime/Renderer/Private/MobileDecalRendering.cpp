...
void RenderDeferredDecalsMobile(FRHICommandList& RHICmdList, const FScene& Scene, const FViewInfo& View, EDecalRenderStage DecalRenderStage, EDecalRenderTargetMode RenderTargetMode)
{
	...
	if (SortedDecals.Num() > 0)
	{
		...
		for (int32 DecalIndex = 0; DecalIndex < SortedDecals.Num(); DecalIndex++)
		{
			...
			// Set shader params
			// BEGIN META SECTION - Decal Rendering VS Compute Reduction
			DecalRendering::SetShader(RHICmdList, GraphicsPSOInit, 0, View, VisibleDecal, DecalRenderStage, FrustumComponentToClip, UE_ARRAY_COUNT(FrustumComponentToClip), &Scene);
			// END META SECTION - Decal Rendering VS Compute Reduction
