...
void AddBasePassPixelShader(bool bEnableSkyLight, bool bEnableXRSoftOcclusions, bool bIsDebug, FMaterialShaderTypes& OutShaderTypes, bool bIsForOITPass = false)
{
	int32 PermutationId = 0;
...
)
{
	...
	if (PixelShader)
	{
		...
		// BEGIN META SECTION - XR Soft Occlusions
		case GBL_Default:
			AddBasePassPixelShader<LightMapPolicyType, GBL_Default>(bEnableSkyLight, bEnableXRSoftOcclusions, bIsDebug, ShaderTypes, bIsForOITPass);
			break;
		case GBL_ForceVelocity:
			AddBasePassPixelShader<LightMapPolicyType, GBL_ForceVelocity>(bEnableSkyLight, bEnableXRSoftOcclusions, bIsDebug, ShaderTypes, bIsForOITPass);
			break;
		// END META SECTION - XR Soft Occlusions
		default:
			check(false);
