	...
	const FCompositePrimitiveInputs& Inputs)
{
	...
	FRDGTextureRef DebugPrimitiveDepth = CreateCompositeDepthTexture(GraphBuilder, Extent, NumMSAASamples, View.bIsMobileMultiViewEnabled);
		...
		[&View, PassParameters, DebugView, OutputViewport](FRDGAsyncTask, FRHICommandList& RHICmdList)
	{
		...
		{
			RHICmdList.SetStereoViewport(OutputViewport.Rect.Min.X, OutputViewport.Rect.Min.X, OutputViewport.Rect.Min.Y, OutputViewport.Rect.Min.Y, 0.0f, OutputViewport.Rect.Max.X, OutputViewport.Rect.Max.X, OutputViewport.Rect.Max.Y, OutputViewport.Rect.Max.Y, 1.0f);
			RHICmdList.SetStereoScissor(OutputViewport.Rect.Min.X, OutputViewport.Rect.Min.X, OutputViewport.Rect.Min.Y, OutputViewport.Rect.Min.Y, OutputViewport.Rect.Max.X, OutputViewport.Rect.Max.X, OutputViewport.Rect.Max.Y, OutputViewport.Rect.Max.Y);
		}
