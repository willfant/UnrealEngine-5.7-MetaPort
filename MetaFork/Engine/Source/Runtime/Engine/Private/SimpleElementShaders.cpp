...
void FSimpleElementVS::SetParameters(FRHIBatchedShaderParameters& BatchedParameters, const FMatrix& WorldToClipMatrix)
{
	...
	const FMatrix44f FinalMats[2] = { FMatrix44f(WorldToClipMatrix) , FMatrix44f(WorldToClipMatrix) };
	const FVector4f PositionsHigh[2] = { FVector3f::ZeroVector, FVector3f::ZeroVector };
	SetShaderValue(BatchedParameters, RelativeTransform, FinalMats);
	SetShaderValue(BatchedParameters, TransformPositionHigh, PositionsHigh);
	// END META SECTION - Multiview Debug Primitives
}

// BEGIN META SECTION - Multiview Debug Primitives
void FSimpleElementVS::SetParameters(FRHIBatchedShaderParameters& BatchedParameters, const FDFRelativeViewMatrices Matrices[2])
{
	const FMatrix44f FinalMats[2] = { Matrices[0].RelativeWorldToClip , Matrices[1].RelativeWorldToClip };
	const FVector4f PositionsHigh[2] = { Matrices[0].PositionHigh, Matrices[1].PositionHigh };
	SetShaderValue(BatchedParameters, RelativeTransform, FinalMats);
	SetShaderValue(BatchedParameters, TransformPositionHigh, PositionsHigh);
}
// END META SECTION - Multiview Debug Primitives
