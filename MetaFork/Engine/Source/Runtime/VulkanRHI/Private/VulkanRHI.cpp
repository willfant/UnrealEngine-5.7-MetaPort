...
{
	static const uint32 PrecacheHashVersion = 4;
...
uint64 FVulkanDynamicRHI::RHIComputeStatePrecachePSOHash(const FGraphicsPipelineStateInitializer& Initializer)
{
	...
	// For hotfix 5.6.1, bAllowVariableRateShading is ignored for precaching hashes because it is not reflected in Pipeline File Cache entries
	// This previously led cache reads for PSOs with bAllowVariableRateShading=false to be directed to the wrong chunk in the Vulkan Chunked Pipeline Cache and miss
	// This fix will still result in a miss for drivers where toggling bAllowVariableRateShading requires a PSO recompile
	HashKey.bAllowVariableRateShading = true;
