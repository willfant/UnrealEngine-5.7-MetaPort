...
namespace {
	...
	/** Hotfix 5.6.1: Helper function to run OpenXR calls which may access VkQueue on the Vulkan submission thread */
	void RunOnGraphicsQueue(TFunction<void()>&& CodeToRun)
	{
#ifdef XR_USE_GRAPHICS_API_VULKAN
		if (RHIGetInterfaceType() == ERHIInterfaceType::Vulkan)
		{
			GetIVulkanDynamicRHI()->RHIRunOnQueue(EVulkanRHIRunOnQueueType::Graphics, [CodeToRun](VkQueue Queue)
			{
				FTaskTagScope Scope(ETaskTag::EParallelRhiThread);
				CodeToRun();
			}, false);
		}
		else
...
void FOpenXRHMD::GetMotionControllerData(UObject* WorldContext, const EControllerHand Hand, FXRMotionControllerData& MotionControllerData)
{
	...
	{
		MotionControllerData.DeviceName = FName(InteractionProfile);
	}
...
// BEGIN META SECTION - Add headlock support
	if (DeviceSpaces[DeviceId].OrientationTracked)
	{
		CurrentOrientation = ToFQuat(Location.pose.orientation);
	}
	if (DeviceSpaces[DeviceId].PositionTracked)
	{
		CurrentPosition = ToFVector(Location.pose.position, GetWorldToMetersScale());
	}
	...
	, bxrGetSystemPropertiesSuccessful(false)
{
	...
	bCubemapLayersSupported = IsExtensionEnabled(XR_KHR_COMPOSITION_LAYER_CUBE_EXTENSION_NAME);
	...
	if (PipelineState.bXrFrameStateUpdated && PipelineState.TrackingSpace.IsValid())
	{
		...
		for (int32 DeviceIndex = 0; DeviceIndex < PipelineState.DeviceLocations.Num(); DeviceIndex++)
		{
			...
			if (DeviceSpace.Space != XR_NULL_HANDLE)
			{
				...
				{
					// BEGIN META SECTION - Add headlock support
					if(DeviceSpaces[DeviceIndex].PositionTracked)
					{
						// Clear the position tracked bits
						CachedDeviceLocation.locationFlags &= ~XR_SPACE_LOCATION_POSITION_TRACKED_BIT;
					...
					if(DeviceSpaces[DeviceIndex].OrientationTracked)
					{
						// Clear the orientation tracked bits
						CachedDeviceLocation.locationFlags &= ~XR_SPACE_LOCATION_ORIENTATION_TRACKED_BIT;
...
// BEGIN META SECTION - Add headlock support
void FOpenXRHMD::SetTrackingPositionEnabled(const int32 DeviceId, bool bEnable)
{
	FReadScopeLock DeviceLock(DeviceMutex);
	if (DeviceSpaces.IsValidIndex(DeviceId))
	{
		DeviceSpaces[DeviceId].PositionTracked = bEnable;
	}
}

void FOpenXRHMD::SetTrackingOrientationEnabled(const int32 DeviceId, bool bEnable)
{
	FReadScopeLock DeviceLock(DeviceMutex);
	if (DeviceSpaces.IsValidIndex(DeviceId))
	{
		DeviceSpaces[DeviceId].OrientationTracked = bEnable;
	}
}
// END META SECTION - Add headlock support
...
void FOpenXRHMD::AllocateDepthTextureInternal(uint32 SizeX, uint32 SizeY, uint32 NumSamples, uint32 InArraySize)
{
	...
	FReadScopeLock Lock(SessionHandleMutex);
	// BEGIN META SECTION - Toggle Depth at Runtime
	if (!Session || !bDepthExtensionSupported || !CheckPlatformDepthExtensionSupport(InstanceProperties))
	// END META SECTION - Toggle Depth at Runtime
	{
		return;
	}
...
// BEGIN META SECTION - Add XR Cubemap Support
			Texture->GetDesc().Dimension != SwapchainDimension ||
// END META SECTION - Add XR Cubemap Support
...
// BEGIN META SECTION - Add XR Cubemap Support
			DynamicFlag |= (Texture->GetDesc().Dimension == ETextureDimension::TextureCube) ? TexCreate_TargetArraySlicesIndependently : TexCreate_None;
// END META SECTION - Add XR Cubemap Support
			uint8 UnusedActualFormat = 0;
...
// BEGIN META SECTION - Add XR Cubemap Support
			SwapchainDimension = Texture->GetDesc().Dimension;
// END META SECTION - Add XR Cubemap Support
...
// BEGIN META SECTION - Add XR Cubemap Support
							(Desc.HasShape<FCubemapLayer>() && bCubemapLayersSupported))
// END META SECTION - Add XR Cubemap Support
						{
...
// BEGIN META SECTION - Add XR Cubemap Support
		if (DeprecatedRef && DeprecatedRef->GetDesc().Dimension == Dimension)
		{
...
// BEGIN META SECTION - Add XR Cubemap Support
	ETextureDimension TargetDimension = Layer.Desc.HasShape<FCubemapLayer>() ? ETextureDimension::TextureCube : ETextureDimension::Texture2D;
// END META SECTION - Add XR Cubemap Support
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
...
void FOpenXRHMD::UpdateLayerSwapchainTexture(FOpenXRLayer& Layer, FRDGBuilder& GraphBuilder)
{
	...
	if (Layer.RightEye.Swapchain.IsValid() && Layer.RightEye.Texture.IsValid())
	{
		...
		{
// BEGIN META SECTION - Add XR Cubemap Support
			FRHITexture* SrcTexture = Layer.RightEye.Texture;
// END META SECTION - Add XR Cubemap Support
	...
	if (Layer.LeftEye.Swapchain.IsValid() && Layer.LeftEye.Texture.IsValid())
	{
		...
		{
// BEGIN META SECTION - Add XR Cubemap Support
			FRHITexture* SrcTexture = Layer.LeftEye.Texture;
// END META SECTION - Add XR Cubemap Support
...
void FOpenXRHMD::OnBeginRendering_RenderThread(FRDGBuilder& GraphBuilder, FSceneViewFamily& ViewFamily)
{
	...
	if (bIsRunning && PipelinedFrameStateRendering.bXrFrameStateUpdated)
	{
		...
		GraphBuilder.RHICmdList.EnqueueLambda([this, FrameState = PipelinedFrameStateRendering, ColorSwapchain, DepthSwapchain, EmulationSwapchain](FRHICommandListImmediate& InRHICmdList)
		{
			...
			RunOnGraphicsQueue([this, FrameState = PipelinedFrameStateRendering, ColorSwapchain, DepthSwapchain, EmulationSwapchain, Context]()
			{
				OnBeginRendering_RHIThread(*Context, FrameState, ColorSwapchain, DepthSwapchain, EmulationSwapchain);
			});
		});
	}
...
{
	ensure(IsInRenderingThread() || IsInRHIThread() || IsInParallelRHIThread());
...
{
// BEGIN META SECTION - Add XR Cubemap Support
	const bool bAcquireOnRender = bIsAcquireOnAnyThreadSupported;
	if (bAcquireOnRender)
	{
		DstSwapChain->IncrementSwapChainIndex_RHIThread();
	}

	AddPass(GraphBuilder, RDG_EVENT_NAME("OpenXRHMD_AcquireLayerSwapchain"), [bAcquireOnRender, DstSwapChain](FRHICommandListImmediate& RHICmdList)
	{
		RHICmdList.EnqueueLambda([bAcquireOnRender, DstSwapChain](FRHICommandListImmediate& InRHICmdList)
		{
			RunOnGraphicsQueue([bAcquireOnRender, DstSwapChain]()
			{
				if (!bAcquireOnRender)
				{
					DstSwapChain->IncrementSwapChainIndex_RHIThread();
...
// END META SECTION - Add XR Cubemap Support
	...
	AddPass(GraphBuilder, RDG_EVENT_NAME("OpenXRHMD_ReleaseLayerSwapchain"), [DstSwapChain, bCapturableRequiresRHIContext](FRHICommandListImmediate& RHICmdList)
	{
		...
		{
			RunOnGraphicsQueue([DstSwapChain, bCapturableRequiresRHIContext, &InRHICmdList]()
			{
