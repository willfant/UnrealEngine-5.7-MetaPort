...
void FXRCopyTextureOptions::SetDisplayMappingOptions(IStereoRenderTargetManager* HDRManager)
{
	...
	bool isTonemapEnabled = IsMobileTonemapSubpassEnabledInline(ShaderPlatform, bUnused , 1);
	if (!IsMobileHDR() && IsMobilePlatform(ShaderPlatform) && IsSimulatedPlatform(ShaderPlatform) && !isTonemapEnabled)
	{
...
// BEGIN META SECTION - Add XR Cubemap Support
	bool bIsCubeCopy = DstTexture->Desc.Dimension == ETextureDimension::TextureCube && SrcTexture->Desc.Dimension == ETextureDimension::TextureCube;
	int TotalSlices = bIsArrayCopy ? FMath::Min(DstTexture->Desc.ArraySize, SrcTexture->Desc.ArraySize) :
					bIsCubeCopy ? 6 : 1;
// END META SECTION - Add XR Cubemap Support
...
// BEGIN META SECTION - Add XR Cubemap Support
	...
	bool bIsArraySource = SrcTexture->GetDesc().Dimension == ETextureDimension::Texture2DArray;
	bool bIsCubeSource = SrcTexture->GetDesc().Dimension == ETextureDimension::TextureCube;

	if (bIsCubeSource)
	{
		TShaderMapRef<FOculusOpenXRCubemapPS> ScreenPSRef(ShaderMap);
		CubemapPS = ScreenPSRef;
		PixelShader = ScreenPSRef;
	}
	else if (Options.bNeedsDisplayMapping)
// END META SECTION - Add XR Cubemap Support
	{
...
// BEGIN META SECTION - Add XR Cubemap Support
	...
	{
		check(ArraySlice < UE_ARRAY_COUNT(FaceToSampleMatrix));
		SetShaderParametersLegacyPS(RHICmdList, CubemapPS, PixelSampler, SrcTexture, FaceToSampleMatrix[ArraySlice]);
	}
// END META SECTION - Add XR Cubemap Support
