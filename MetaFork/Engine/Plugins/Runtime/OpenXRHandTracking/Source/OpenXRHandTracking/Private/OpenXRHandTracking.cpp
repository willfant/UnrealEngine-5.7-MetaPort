...
// BEGIN META SECTION - CreateHandTracker Hooks
#include "IOpenXRHMD.h"
// END META SECTION - CreateHandTracker Hooks
...
const void* FOpenXRHandTracking::OnBeginSession(XrSession InSession, const void* InNext)
{
	if (bHandTrackingAvailable)
	{
		...
		CreateHandTracker(InSession, XR_HAND_LEFT_EXT, LeftHandState);

		// Create a hand tracker for right  hand that tracks default set of hand joints.
		FHandState& RightHandState = GetRightHandState();
		CreateHandTracker(InSession, XR_HAND_RIGHT_EXT, RightHandState);
		// END META SECTION - CreateHandTracker Hooks
	}
...
// BEGIN META SECTION - CreateHandTracker Hooks
void FOpenXRHandTracking::CreateHandTracker(XrSession InSession, XrHandEXT HandEXT, FHandState& HandState)
{
	...
	if (XRTrackingSystem && XRTrackingSystem->GetIOpenXRHMD())
	{
		ExtensionPlugins = XRTrackingSystem->GetIOpenXRHMD()->GetExtensionPlugins();
	}
	
	for (const auto Plugin : ExtensionPlugins)
	{
