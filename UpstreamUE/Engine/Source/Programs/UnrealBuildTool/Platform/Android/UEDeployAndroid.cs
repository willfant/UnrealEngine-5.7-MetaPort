...
namespace UnrealBuildTool
{
	class UEDeployAndroid : UEBuildDeploy, IAndroidDeploy
	{
		...
		private const string BUNDLETOOL_JAR = "bundletool-all-1.18.1.jar";

		// classpath of default android build tools gradle plugin
		private const string ANDROID_TOOLS_BUILD_GRADLE_VERSION = "com.android.tools.build:gradle:8.6.1";

		// default NDK version if not set
		private const string DEFAULT_NDK_VERSION = "27.2.12479018";
		...
		bool IAndroidDeploy.bFromMSBuild => bFromMSBuild;
		...
		/// </summary>
		[CommandLine("-FastIterate", Value = "true")]
		public bool bFastIterate = false;
		bool IAndroidDeploy.bFastIterate => bFastIterate;
		
		/// <summary>
		/// Only generates gradle 
		/// </summary>
		[CommandLine("-OnlyGradleGeneration", Value = "true")]
		public bool bOnlyGradleGeneration = false;
		...
		{
			int DashIndex = ApiString.LastIndexOf('-');
			if (DashIndex != -1 && int.TryParse(ApiString.Substring(DashIndex + 1), out int Version))
			{
				return Version;
			}
			
			Logger.LogWarning("Unable to parse SDK API level '{ApiString}', it is expected to be of the form 'android-NN'", ApiString);
			return 0;
		}
		...
		public static void CopyFileDirectory(string SourceDir, string DestDir, Dictionary<string, string>? Replacements = null, string[]? Excludes = null)
		{
		...
		public static void DeleteDirectory(string InPath, ILogger Logger, string SubDirectoryToKeep = "")
		{
		...
		private static bool IsSO16KbCompatible(string FileName, UnrealArch UnrealArch)
		{
			string ReadElfExecutable = AndroidToolChain.GetReadElfExecutablePath(UnrealArch);
			string StdOutput = Utils.RunLocalProcessAndReturnStdOut(ReadElfExecutable, $"--elf-output-style=JSON --program-headers \"{FileName}\"");

			ReadElfOutput Output = default;
			bool bParsedOk = false;
			try
			{
				// expected in NDK >= 27
				List<ReadElfOutput>? JsonOutput = JsonSerializer.Deserialize<List<ReadElfOutput>>(StdOutput);
				if (JsonOutput != null)
				{
					Output = JsonOutput.First();
					bParsedOk = true;
				}
			}
			catch (Exception)
			...
			if (!bParsedOk)
			{
				...
				{
					// expected in NDK 25
					List<Dictionary<string, ReadElfOutput>>? JsonOutput = JsonSerializer.Deserialize<List<Dictionary<string, ReadElfOutput>>>(StdOutput);
					if (JsonOutput != null)
					{
						Output = JsonOutput.First().First().Value;
						bParsedOk = true;
					}
				}
				catch (Exception)
			...
			if (!bParsedOk || Output.ProgramHeaders == null || Output.ProgramHeaders.Count == 0)
			{
				// don't fail the build if we can't check alignment
				return true;
			}

			bool bIsAlignmentOk = true;

			foreach (ReadElfProgramHeaderWrapper Wrapper in Output.ProgramHeaders)
			{
				ReadElfProgramHeader ProgramHeader = Wrapper.ProgramHeader;

				if (ProgramHeader.Type.Name == "PT_LOAD")
				{
					bIsAlignmentOk &= ProgramHeader.Alignment >= 16 * 1024;
				}
			}

			return bIsAlignmentOk;
		}

		private struct ReadElfOutput
		{
			public required List<ReadElfProgramHeaderWrapper> ProgramHeaders { get; set; }
		}

		private struct ReadElfProgramHeaderWrapper
		{
			public ReadElfProgramHeader ProgramHeader { get; set; }
		}

		private struct ReadElfProgramHeader
		{
			public ReadElfProgramHeaderType Type { get; set; }
			public int Alignment { get; set; }
		}

		private struct ReadElfProgramHeaderType
		{
			// only present in NDK >=27
			[JsonPropertyName("Name")]
			public string MaybeName { get; set; }

			// in NDK 25 this is a string, in NDK >=27 it's an int
			[JsonPropertyName("Value")]
			public object MaybeValue { get; set; }

			private string MaybeValueAsString => MaybeValue?.ToString() ?? string.Empty;

			[JsonIgnore]
			public string Name => !string.IsNullOrEmpty(MaybeName) ? MaybeName : MaybeValueAsString;
		}

		private void CheckSOInFolderAre16KbCompatible(string FolderPath, UnrealArch UnrealArch)
		{
			try
			{
				IEnumerable<string> SOFiles = Directory.EnumerateFiles(FolderPath, "*.so", SearchOption.AllDirectories);

				foreach (var SOFile in SOFiles)
				{
					if (!IsSO16KbCompatible(SOFile, UnrealArch))
					{
						Logger.LogWarning("Library '{SOFile}' is NOT compatible with 16kb page size alignment.", SOFile);
					}
				}
			}
			catch (Exception)
		...
		private void CheckSOInGradleFolderAre16KbCompatible(string GradlePath, UnrealArch UnrealArch)
		{
			CheckSOInFolderAre16KbCompatible($"{GradlePath}/jniLibs/arm64-v8a", UnrealArch);
			CheckSOInFolderAre16KbCompatible($"{GradlePath}/libs/arm64-v8a", UnrealArch);
		}
		...
		private string GetAllBuildSettings(AndroidToolChain ToolChain, UnrealPluginLanguage UPL, bool bForDistribution, bool bMakeSeparateApks, bool bPackageDataInsideApk,
			bool bDisableVerifyOBBOnStartUp, bool bUseExternalFilesDir, bool bEnableBundle, bool bDontBundleLibrariesInAPK, bool bUsesZenStreaming, string TemplatesHashCode)
		{
			...
			CurrentSettings.AppendLine(String.Format("bEnableBundle={0}", bEnableBundle));
		...
		private string GenerateManifest(AndroidToolChain ToolChain, string ProjectName, TargetType InTargetType, string EngineDirectory, bool bIsForDistribution, bool bPackageDataInsideApk, string GameBuildFilesPath, bool bHasOBBFiles, bool bDisableVerifyOBBOnStartUp, UnrealArch UnrealArch, string CookFlavor, bool bUseExternalFilesDir, string Configuration, int SDKLevelInt, bool bIsEmbedded, bool bEnableBundle, bool bUsesZenStreaming)
		{
			...
			string RoundedEdgeSafeZone;
			Ini.GetString("/Script/AndroidRuntimeSettings.AndroidRuntimeSettings", "RoundedEdgeSafeZone", out RoundedEdgeSafeZone);
			bool bAllowResizing;
			...
			else
			{
				...
				Text.AppendLine("\t\t          android:exported=\"true\"");
				Text.AppendLine("\t\t          android:label=\"@string/app_name\"");
				Text.AppendLine("\t\t          android:theme=\"@style/UnrealBaseTheme\"");
				Text.AppendLine(bAddDensity ? "\t\t          android:configChanges=\"mcc|mnc|uiMode|density|screenSize|smallestScreenSize|screenLayout|orientation|keyboardHidden|keyboard|navigation|touchscreen|locale|fontScale|layoutDirection\""
											: "\t\t          android:configChanges=\"mcc|mnc|uiMode|screenSize|smallestScreenSize|screenLayout|orientation|keyboardHidden|keyboard|navigation|touchscreen|locale|fontScale|layoutDirection\"");
			...
			if (TargetSDKVersion >= 23 && TargetSDKVersion < 30 && !bPackageForMetaQuest)
			{
			...
			Text.AppendLine(String.Format("\t\t<meta-data android:name=\"com.epicgames.unreal.GameActivity.RoundedEdgeSafeZone\" android:value=\"{0}\"/>", RoundedEdgeSafeZone));
			...
			else
			{
				...
				// Apps requesting these permissions are rejected by the Quest App Store
				if (!bPackageForMetaQuest)
				{
					if (TargetSDKVersion >= 23 && TargetSDKVersion < 30)
					{
						if (Configuration != "Shipping" || !bUseExternalFilesDir)
						{
		...
		{
			if (bFastIterate || ForceAPKGeneration)
			{
				return false;
			}
			ConfigHierarchy Ini = GetConfigCacheIni(ConfigHierarchyType.Engine);
...
";
		...
		public bool GetDontBundleLibrariesInAPK(UnrealTargetConfiguration Configuration, bool bIsArchive, bool bIsFromUAT)
		{
			if (ForceDontBundleLibrariesInAPK.HasValue)
			{
				Logger?.LogInformation("bDontBundleLibrariesInAPK is force set to {ForceDontBundleLibrariesInAPK} via command line option.", ForceDontBundleLibrariesInAPK.Value);
				return ForceDontBundleLibrariesInAPK.Value;
			}
			...
			// Deprecated in 5.7, remove everything below in 5.9 and just return bFastIterate
			...
			UnrealTargetConfiguration Configuration, bool bMakeSeparateApks, bool bIncrementalPackage, bool bDisallowPackagingDataInApk, bool bDisallowExternalFilesDir, bool bSkipGradleBuild, bool bIsArchive, bool bIsFromUAT)
		{
			...
			if (!bFastIterate && !ForceAPKGeneration && ProjectName == "UnrealGame" && OutputPath.Replace("\\", "/").Contains("/Engine/Binaries/Android/") && Path.GetFileNameWithoutExtension(OutputPath).StartsWith("UnrealGame"))
			{
				if (!bSkipGradleBuild)
				{
					/*
			...
			// Check if bEnableBundle changed
			...
			if (!bCreateFromScratch)
			{
				...
				{
					const string Key = "bEnableBundle=";
					string? Line = File.ReadAllLines(BuildSettingsCacheFile).FirstOrDefault(Line => Line.StartsWith(Key));
					if (Line != null && !Line[Key.Length..].Equals(bEnableBundle ? "True" : "False"))
					{
						bCreateFromScratch = true;
					}
				}
			...
			bool bDontBundleLibrariesInAPK = GetDontBundleLibrariesInAPK(Configuration, bIsArchive, bIsFromUAT);
			...
			string CurrentBuildSettings = GetAllBuildSettings(ToolChain, UPL!, bForDistribution, bMakeSeparateApks, bPackageDataInsideApk, bDisableVerifyOBBOnStartUp, bUseExternalFilesDir, bEnableBundle, bDontBundleLibrariesInAPK, bUsesZenStreaming, TemplatesHashCode);
			...
			if (!bBuildSettingsMatch || bOnlyGradleGeneration)
			{
				// LINQ query is subsequently used multiple times, so must copy results
			...
			if (bDontBundleLibrariesInAPK)
			{
				foreach ((UnrealArch Arch, string _) in Archs)
				{
					...
					FileInfo SOFileStripped = new(SONameStripped);
					if (!SOFileStripped.Exists || SOFileStripped.CreationTime != SOFile.CreationTime || SOFileStripped.LastWriteTime != SOFile.LastWriteTime)
					{
						StripDebugSymbols(SOName, SONameStripped, Arch, Logger, true);
						SOFileStripped.CreationTime = SOFile.CreationTime;
						SOFileStripped.LastWriteTime = SOFile.LastWriteTime;
					}
			...
			else
			{
				foreach ((UnrealArch Arch, string _) in Archs)
				{
					...
					SafeDeleteFile(SONameStripped);
					SafeDeleteFile(Path.Combine(LLDBExtraSymbolsDirectory, "libUnreal.so"));
				}
			}
			...
			if (BuildListComboTotal == 0 && !bOnlyGradleGeneration)
			{
				Logger.LogInformation("Output .apk file(s) are up to date (dependencies and build settings are up to date)");
				return;
			}

			if (BypassGradlePackaging && !bEnableBundle && !bSkipGradleBuild && !bOnlyGradleGeneration && !bCreateFromScratch)
			{
			...
			foreach (Tuple<UnrealArch, string> build in BuildList)
			{
				...
				if (bPackageDataInsideApk)
				{
					...
					if (File.Exists(ObbFileLocation))
					{
						...
						bInApkAssetChange = CopyIfDifferent(SrcFileName, DestFileName, true, false);
					}
				}
				else // try to remove the file it we aren't packaging inside the APK
				{
					...
					bInApkAssetChange = SafeDeleteFile(DestFileName);
				}

				if (bInApkAssetChange)
				{
					// Workaround for Gradle bug whereby a change in the contents of assets triggers a patch build whereby main.obb.png is duplicated in the APK causing the APK to grow in size. 
					// Workaround deletes existing Gradle output APK to rebuild when main.obb.png is modified or deleted.
					string GradleConfig = bForDistribution ? "release" : "debug";
					string GradeOutAPK = Path.Combine(UnrealBuildGradlePath, "app", "build", "outputs", "apk", GradleConfig, "app-" + GradleConfig + ".apk");
					SafeDeleteFile(GradeOutAPK);
				}
				...
				else if (bDontBundleLibrariesInAPK)
				{
					...
					if (!bOnlyGradleGeneration && !File.Exists(SourceSONameStripped))
					{
					...
					string AFSExecutable = Path.Combine(Unreal.EngineDirectory.ToString(), @"Binaries/DotNET/Android/UnrealAndroidFileTool", GetAFSExecutable(BuildHostPlatform.Current.Platform, Logger));
...
";
				...
				else
				{
					...
					if (!bOnlyGradleGeneration && !File.Exists(SourceSOName))
					{
						throw new BuildException("Can't make an APK without the compiled .so [{0}]", SourceSOName);
					}
					if (!bOnlyGradleGeneration && !Directory.Exists(UnrealBuildPath + "/jni"))
					{
...
//				LocalProperties.AppendLine(string.Format("ndk.dir={0}", Environment.GetEnvironmentVariable("NDKROOT")!.Replace("\\", "/")));
				...
				CheckSOInGradleFolderAre16KbCompatible(UnrealBuildGradleMainPath, Arch);

				if (!bSkipGradleBuild && !bOnlyGradleGeneration)
				{
				...
				bSaveSymbols = true;
				if ((bBuildWithHiddenSymbolVisibility || bSaveSymbols) && !bOnlyGradleGeneration)
				{
					// Copy .so with symbols to 
					int StoreVersion = GetStoreVersion(bEnableBundle ? null : Arch);
					string SymbolSODirectory = Path.Combine(DestApkDirectory, ProjectName + "_Symbols_v" + StoreVersion, ProjectName + Arch);
			...
			if (bEnableBundle && !bSkipGradleBuild && !bOnlyGradleGeneration)
			{
...
";
				...
				bool bJavaPerfettoMarkers, UnrealTargetConfiguration Configuration, string SOName)
		{
			...
			Dictionary<string, string> Replacements = new Dictionary<string, string>{
				...
				{ "//$${gameApplicationImplementsAdditions}$$", UPL!.ProcessPluginNode(NDKArch, "gameApplicationImplementsAdditions", "")},
		...
		private void PrepareJavaLibsForGradle(string JavaLibsDir, string UnrealBuildGradlePath, string InMinSdkVersion, string InTargetSdkVersion, string CompileSDKVersion, string BuildToolsVersion, string NDKArch)
		{
			...
			foreach (string LibDir in LibDirs)
			{
				...
				BuildGradleContent.AppendLine("\tcompileOptions {");
				...
				BuildGradleContent.AppendLine("\t\tcoreLibraryDesugaringEnabled true");
				BuildGradleContent.AppendLine("\t}");
				BuildGradleContent.AppendLine("\tdefaultConfig {");
				BuildGradleContent.AppendLine("\t\tmultiDexEnabled true");
				...
				BuildGradleContent.AppendLine("\t}");
				BuildGradleContent.AppendLine("}");
				BuildGradleContent.AppendLine("dependencies.coreLibraryDesugaring \"com.android.tools:desugar_jdk_libs:2.1.5\"");
