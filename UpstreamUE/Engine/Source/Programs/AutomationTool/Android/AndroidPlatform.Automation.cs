...
public class AndroidPlatform : Platform
{
	...
	private static string GetSdkDir()
	{
		...
		else if (OperatingSystem.IsMacOS())
		{
			...
			string BashProfilePath = Path.Combine(UserProfileDir, ".zshrc");
	...
	private bool UsingAndroidFileServer(ProjectParams Params, DeploymentContext SC, out bool bEnablePlugin, out string AFSToken, out bool bIsShipping, out bool bIncludeInShipping, out bool bAllowExternalStartInShipping)
	{
		...
		if (Params.ProjectTargets.Any(Target => Target.Rules.IsTestTarget))
		{
			bEnablePlugin = true;
			AFSToken = "";
	...
	*/
	...
	private bool WasBuiltWithDontBundleLibrariesInAPK(string FinalSOName)
	{
		// The existence of the Push_..._so.bat file signifies a DontBundleLibrariesInAPK build
		return File.Exists(Path.Combine(Path.GetDirectoryName(FinalSOName)!, "Push_" + Path.GetFileNameWithoutExtension(FinalSOName) + "_so.bat"));
	}
	...
	private void DeployAndroidFileServer(ProjectParams Params, DeploymentContext SC, string AFSToken)
	{
		...
		IAndroidDeploy Deploy = AndroidExports.CreateDeploymentHandler(Params.RawProjectPath, Params.ForcePackageData);
		bool bDontBundleLibrariesInAPK = Deploy.GetDontBundleLibrariesInAPK(SC.StageTargets[0].Receipt.Configuration, SC.Archive, true);
		...
		foreach (string DeviceName in Params.DeviceNames.Select(DeviceName => GetSerialNumber(DeviceName)))
		{
			...
			if (!Params.Prebuilt && (Params.Build || !Deploy.bFastIterate))
			{
				string CookFlavor = SC.FinalCookPlatform.IndexOf("_") > 0 ? SC.FinalCookPlatform.Substring(SC.FinalCookPlatform.IndexOf("_")) : "";
				string SOName = GetSONameWithoutArchitecture(Params, SC.StageExecutables[0]);
			...
			if (Params.IterativeDeploy && !Deploy.bFromMSBuild)
			{
			...
			else
			{
				...
				client.SetActive(PackageName, AFSToken, IPAddress);
			}
			...
			if (bNeedAPKInstall && !Deploy.bFromMSBuild)
			{
				client.FileWriteString("APK: " + APKLastUpdateTime + "\n", "^ext/APKFileStamp.txt");
			}

			if (bDontBundleLibrariesInAPK && WasBuiltWithDontBundleLibrariesInAPK(FinalSOName))
			{
				...
				long NewTimestamp = ((DateTimeOffset)File.GetLastWriteTimeUtc(FinalSONameStripped)).ToUnixTimeMilliseconds();
				
				if (!long.TryParse(client.FileReadString("^int/libUnreal.so.timestamp") ?? "", out long OldTimestamp) || OldTimestamp != NewTimestamp)
				{
					Logger.LogInformation("libUnreal.so changed...pushing to device");
					client.PushFile(FinalSONameStripped, "^int/libUnreal.so");
					client.FileWriteString(NewTimestamp.ToString(), "^int/libUnreal.so.timestamp");
				}
			}
			else
			{
				client.FileDelete("^int/libUnreal.so.timestamp");
				client.FileDelete("^int/libUnreal.so");
			}
	...
	private void DeployADB(ProjectParams Params, DeploymentContext SC)
    {
		...
		foreach (string DeviceName in Params.DeviceNames.Select(DeviceName => GetSerialNumber(DeviceName)))
		{
			...
			if (bDontBundleLibrariesInAPK && WasBuiltWithDontBundleLibrariesInAPK(FinalSOName))
			{
				...
				long NewTimestamp = ((DateTimeOffset)File.GetLastWriteTimeUtc(FinalSOPathStripped)).ToUnixTimeMilliseconds();
				...
				if (!long.TryParse(RunAdbCommand(Params, DeviceName, "shell cat ./files/libUnreal.so.timestamp", null, ERunOptions.AppMustExist).Output, out long OldTimestamp) || OldTimestamp != NewTimestamp)
				{
					...
					RunAndLogAdbCommand(Params, DeviceName, $"shell \"echo '{NewTimestamp}' > ./files/libUnreal.so.timestamp\"", out SuccessCode);
				}
			}
			else
			{
				int SuccessCode;
				RunAndLogAdbCommand(Params, DeviceName, "shell rm ./files/libUnreal.so.timestamp", out SuccessCode);
				RunAndLogAdbCommand(Params, DeviceName, "shell rm ./files/libUnreal.so", out SuccessCode);
			}
