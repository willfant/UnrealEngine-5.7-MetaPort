...
TRDGUniformBufferRef<FDebugViewModePassUniformParameters> CreateDebugViewModePassUniformBuffer(FRDGBuilder& GraphBuilder, const FViewInfo& View, FRDGTextureRef DebugAuxTexture)
{
	FRDGTextureRef QuadOverdrawTexture = nullptr;

	EDebugViewShaderMode DebugViewMode = View.Family->GetDebugViewShaderMode();
	if (DebugViewMode == EDebugViewShaderMode::DVSM_QuadComplexity || DebugViewMode == DVSM_ShaderComplexityContainedQuadOverhead)
	{
		QuadOverdrawTexture = DebugAuxTexture;
		DebugAuxTexture = nullptr;
	}
...
void ClearDebugAux(FRDGBuilder& GraphBuilder, const FViewFamilyInfo& ViewFamily, FRDGTextureRef DebugAuxTexture)
{
	if (DebugAuxTexture)
	{
		...
		FRDGTextureUAVRef DebugAuxTextureUAV = GraphBuilder.CreateUAV(DebugAuxTexture);

		if (DebugViewMode == DVSM_QuadComplexity || DebugViewMode == DVSM_ShaderComplexityContainedQuadOverhead)
		{
			AddClearUAVPass(GraphBuilder, DebugAuxTextureUAV, FUintVector4(0, 0, 0, 0));
		}
		else if (DebugViewMode == DVSM_ShadowCasters)
		{
			const uint32 InvalidInstanceId = 0xFFFFFFFF;
			AddClearUAVPass(GraphBuilder, DebugAuxTextureUAV, FUintVector4(InvalidInstanceId, 0, 0, 0));
		}
	}
}
...
void RenderDebugViewMode(FRDGBuilder& GraphBuilder, TArrayView<FViewInfo> Views, FRDGTextureRef DebugAuxTexture, const FRenderTargetBinding& SceneColorRTBinding, const FDepthStencilBinding& SceneDepthBinding)
{
	...
	check(SceneColorRTBinding.GetTexture());
	...
	FIntPoint SceneColorExtent = SceneColorRTBinding.GetTexture()->Desc.Extent;
	
	// Dummy needs to have the correct size and type to prevent validation errors, but we can use R8 instead of R32.
	const FRDGTextureDesc RenderTarget1Desc = FRDGTextureDesc::Create2D(SceneColorExtent, PF_R8_UINT, FClearValueBinding::None, TexCreate_ShaderResource | TexCreate_RenderTargetable | TexCreate_UAV);
	FRDGTextureRef DummyRenderTarget1 = GraphBuilder.CreateTexture(RenderTarget1Desc, TEXT("DummyDebugAux"));

	FRenderTargetBindingSlots DebugRenderTargets{};
	DebugRenderTargets.DepthStencil = SceneDepthBinding;
	DebugRenderTargets[0] = SceneColorRTBinding;
	DebugRenderTargets[1] = FRenderTargetBinding(DummyRenderTarget1, ERenderTargetLoadAction::ENoAction);

	FRDGTextureRef QuadOverdrawTexture = nullptr;

	EDebugViewShaderMode DebugViewMode = Views[0].Family->GetDebugViewShaderMode();
	if (DebugViewMode == EDebugViewShaderMode::DVSM_QuadComplexity || DebugViewMode == DVSM_ShaderComplexityContainedQuadOverhead)
	{
		QuadOverdrawTexture = DebugAuxTexture;
		DebugAuxTexture = nullptr;
	}
	else if (DebugViewMode == EDebugViewShaderMode::DVSM_ShadowCasters)
	{
		DebugRenderTargets[1] = FRenderTargetBinding(DebugAuxTexture, ERenderTargetLoadAction::ELoad);
	}
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		if (auto* Pass = View.ParallelMeshDrawCommandPasses[EMeshPass::DebugViewMode])
		{
			...
			PassParameters->RenderTargets = DebugRenderTargets;
	...
	: FMeshMaterialShader(Initializer)
{
	...
	PrimitiveBlendModeParameter.Bind(Initializer.ParameterMap, TEXT("PrimitiveBlendMode"));
...
void FDebugViewModePS::ModifyCompilationEnvironment(const FMaterialShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
{
	...
	OutEnvironment.SetDefine(TEXT("BLEND_Opaque"), (int)BLEND_Opaque);
	OutEnvironment.SetDefine(TEXT("BLEND_Masked"), (int)BLEND_Masked);
	...
	FMeshDrawSingleShaderBindings& ShaderBindings) const
{
	...
	ShaderBindings.Add(Shader.PrimitiveBlendModeParameter, (int)Material.GetBlendMode());
