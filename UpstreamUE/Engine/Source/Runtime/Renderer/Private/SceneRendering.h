...
#if RHI_RAYTRACING
private:
	FRayTracingScene::FViewHandle RayTracingSceneViewHandle;

public:

	bool HasRayTracingScene() const;
	FRHIRayTracingScene* GetRayTracingSceneChecked(ERayTracingSceneLayer Layer) const;
	FRDGBufferSRVRef GetRayTracingSceneLayerViewChecked(ERayTracingSceneLayer Layer) const;
	...
	inline void SetRayTracingSceneViewHandle(const FRayTracingScene::FViewHandle& InHandle)
	{
		checkf(!IStereoRendering::IsStereoEyePass(StereoPass) || !IStereoRendering::IsASecondaryPass(StereoPass), TEXT("Stereo views should share a single ray tracing scene view."));
		RayTracingSceneViewHandle = InHandle;
	}
	...
	bool bOwnsShaderBindingTables = true;
	bool bTransientShaderBindingTables = false;
