...
int32 FOcclusionQueryHelpers::GetNumBufferedFrames(ERHIFeatureLevel::Type FeatureLevel)
{
	...
	if ((FeatureLevel <= ERHIFeatureLevel::ES3_1 || IsVulkanMobileSM5Platform(ShaderPlatform) || IsMetalMobileSM5Platform(ShaderPlatform)) && CVarMobileEnableOcclusionExtraFrame.GetValueOnAnyThread())
	{
		...
		bool bNeedsAnotherExtraMobileFrame = IsVulkanPlatform(ShaderPlatform) || IsMetalPlatform(ShaderPlatform) || IsOpenGLPlatform(ShaderPlatform);
		bNeedsAnotherExtraMobileFrame = bNeedsAnotherExtraMobileFrame || IsVulkanMobileSM5Platform(ShaderPlatform) || IsMetalMobileSM5Platform(ShaderPlatform);
	...
	FGraphicsPipelineStateInitializer* InGraphicsPSOInit = nullptr)
{
	...
	FGraphicsPipelineStateInitializer StackGraphicsPSOInit;
	if (InGraphicsPSOInit == nullptr)
	{
		RHICmdList.ApplyCachedRenderTargets(StackGraphicsPSOInit);
		StackGraphicsPSOInit.PrimitiveType = PT_TriangleList;
		StackGraphicsPSOInit.BlendState = TStaticBlendStateWriteMask<CW_NONE, CW_NONE, CW_NONE, CW_NONE, CW_NONE, CW_NONE, CW_NONE, CW_NONE>::GetRHI();
		// Depth tests, no depth writes, no color writes, opaque
		StackGraphicsPSOInit.DepthStencilState = TStaticDepthStencilState<false, CF_DepthNearOrEqual>::GetRHI();
		StackGraphicsPSOInit.BoundShaderState.VertexDeclarationRHI = GetVertexDeclarationFVector3();
	}
	FGraphicsPipelineStateInitializer& GraphicsPSOInit = (InGraphicsPSOInit != nullptr) ? *InGraphicsPSOInit : StackGraphicsPSOInit;
	...
	if (ViewQuery.bFlushQueries)
	{
		...
		if (FeatureLevel == ERHIFeatureLevel::ES3_1 &&
			View.ViewState && View.ViewState->OcclusionFeedback.IsInitialized())
		{
			SCOPED_DRAW_EVENT(RHICmdList, IndividualQueries);
	...
	uint32 DownsampleFactor)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
		...
		BeginOcclusionTests(RHICmdList, View, ViewQuery, FeatureLevel, DownsampleFactor, &GraphicsPSOInit);
	}
}
...
void FMobileSceneRenderer::RenderOcclusion(FRHICommandList& RHICmdList, FViewInfo& View, const FViewOcclusionQueries& ViewOcclusionQueries)
{
	SCOPED_NAMED_EVENT(FMobileSceneRenderer_BeginOcclusionTests, FColor::Emerald);
	BeginOcclusionTests(RHICmdList, View, ViewOcclusionQueries, View.GetFeatureLevel(), 1.0f);
}
