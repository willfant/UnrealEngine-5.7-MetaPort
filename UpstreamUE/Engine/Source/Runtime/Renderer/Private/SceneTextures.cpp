...
static bool FindStereoMotionVectorTexture(FIntPoint& OutVelocityTextureSize, FTextureRHIRef& OutVelocityTexture, FTextureRHIRef& OutVelocityDepthTexture)
{
	if (IStereoRenderTargetManager* StereoRenderTargetManager = FindStereoRenderTargetManager())
	{
		if (StereoRenderTargetManager->GetRecommendedMotionVectorTextureSize(OutVelocityTextureSize))
		{
			if (StereoRenderTargetManager->GetMotionVectorTexture(0, OutVelocityTextureSize, PF_FloatRGBA, 1, TexCreate_RenderTargetable | TexCreate_ResolveTargetable | TexCreate_ShaderResource | TexCreate_InputAttachmentRead, OutVelocityTexture))
			{
				StereoRenderTargetManager->GetMotionVectorDepthTexture(0, OutVelocityTextureSize, PF_DepthStencil, 1, TexCreate_DepthStencilTargetable | TexCreate_ShaderResource | TexCreate_InputAttachmentRead, OutVelocityDepthTexture);
				return true;
			}
		}
	}
...
void FMinimalSceneTextures::InitializeViewFamily(FRDGBuilder& GraphBuilder, FViewFamilyInfo& ViewFamily)
{
	...
	else
	{
		...
		Desc.AliasableFormats.Add(PF_X24_G8);
...
void FSceneTextures::InitializeViewFamily(FRDGBuilder& GraphBuilder, FViewFamilyInfo& ViewFamily, FIntPoint FamilySize)
{
	...
	if (ViewFamily.EngineShowFlags.StereoMotionVectors && PlatformSupportsOpenXRMotionVectors(ViewFamily.GetShaderPlatform()))
	{
		FTextureRHIRef StereoMotionVectorRHI, StereoMotionVectorDepthRHI;
		FIntPoint StereoMotionVectorSize;
		if (FindStereoMotionVectorTexture(StereoMotionVectorSize, StereoMotionVectorRHI, StereoMotionVectorDepthRHI))
		{
			SceneTextures.StereoMotionVectors = RegisterExternalTexture(GraphBuilder, StereoMotionVectorRHI, TEXT("ExternalStereoMotionVectors"));
			if (StereoMotionVectorDepthRHI)
			{
				SceneTextures.StereoMotionVectorDepth = RegisterExternalTexture(GraphBuilder, StereoMotionVectorDepthRHI, TEXT("ExternalStereoMotionVectorDepth"));
			}
		}
	}
	...
	if (DoesPlatformSupportNanite(Config.ShaderPlatform))
	{
		ETextureCreateFlags FlagsToAdd = TexCreate_None;
		const FGBufferBindings& Bindings = Config.GBufferBindings[GBL_Default];

		const FRDGTextureDesc Desc = FRDGTextureDesc::CreateRenderTargetTextureDesc(Config.Extent, PF_A2B10G10R10, FClearValueBinding::Black, Bindings.GBufferA.Flags | FlagsToAdd | GFastVRamConfig.GBufferA, Config.bRequireMultiView, Config.MobileMultiViewRenderTargetNumLayers);
		SceneTextures.GBufferSGGX = GraphBuilder.CreateTexture(Desc, TEXT("GBufferSGGX"));
	}
#endif

#if WITH_DEBUG_VIEW_MODES
	{
		FIntPoint DebugAuxTextureExtent { 1, 1 };
		EPixelFormat DebugAuxTextureFormat = PF_R32_UINT;

		EDebugViewShaderMode DebugViewMode = ViewFamily.GetDebugViewShaderMode();
		if (DebugViewMode == DVSM_QuadComplexity || DebugViewMode == DVSM_ShaderComplexityContainedQuadOverhead)
		{
			DebugAuxTextureExtent.X = 2 * FMath::Max<uint32>((Config.Extent.X + 1) / 2, 1); // The size is time 2 since left side is QuadDescriptor, and right side QuadComplexity.
			DebugAuxTextureExtent.Y =     FMath::Max<uint32>((Config.Extent.Y + 1) / 2, 1);
		}
		else if (DebugViewMode == DVSM_ShadowCasters)
		{
			DebugAuxTextureExtent = Config.Extent;
		}

		const FRDGTextureDesc DebugAuxTextureDesc(FRDGTextureDesc::Create2D(DebugAuxTextureExtent, DebugAuxTextureFormat, FClearValueBinding::None, TexCreate_ShaderResource | TexCreate_RenderTargetable | TexCreate_UAV));
		SceneTextures.DebugAux = GraphBuilder.CreateTexture(DebugAuxTextureDesc, TEXT("DebugAuxTexture"));
	}
#endif

#if SUPPORTS_VISUALIZE_TEXTURE
	// This function can be called for a Custom Render Pass view family.  Skip those, by checking if this is the
	// main view family (the first view is the same as the first one in "AllViews").
	if (GVisualizeTexture.IsRequestedView() && ViewFamily.Views[0] == ViewFamily.AllViews[0])
	{
...
TArray<FRDGTextureRef> FSceneTextures::EnumerateSceneTextures() const
{
	...
	AddTextureIfNonNull(GBufferSGGX, Results);
	...
	AddTextureIfNonNull(DebugAux, Results);
#if WITH_EDITOR
	...
	FSceneTextureUniformParameters& SceneTextureParameters)
{
	...
	if (SceneTextures)
	{
		...
		if (EnumHasAnyFlags(SetupMode, ESceneTextureSetupMode::SGGX) && HasBeenProduced(SceneTextures->GBufferSGGX))
		{
			SceneTextureParameters.GBufferSGGXTexture = SceneTextures->GBufferSGGX;
		}
