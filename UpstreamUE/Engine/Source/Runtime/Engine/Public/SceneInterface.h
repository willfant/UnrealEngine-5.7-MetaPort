...
public:
	...
	*/
	...
	{
	public:
		virtual ~IPrimitiveTransformUpdater() = default;

		ENGINE_API static IPrimitiveTransformUpdater* SetInstanceTLS(IPrimitiveTransformUpdater* Instance);
		ENGINE_API static IPrimitiveTransformUpdater* GetInstanceTLS();
	};

	class FPrimitiveTransformUpdateScope
	{
	public:
		explicit FPrimitiveTransformUpdateScope(IPrimitiveTransformUpdater* Updater)
		{
			PreviousUpdater = IPrimitiveTransformUpdater::SetInstanceTLS(Updater);
		}

		~FPrimitiveTransformUpdateScope()
		{
			IPrimitiveTransformUpdater::SetInstanceTLS(PreviousUpdater);
		}

	private:
		IPrimitiveTransformUpdater* PreviousUpdater;
	};

	virtual IPrimitiveTransformUpdater* CreatePrimitiveTransformUpdater(int32 MaxPrimitives) { return nullptr; }
	virtual void UpdatePrimitiveTransforms(IPrimitiveTransformUpdater* Updater) {}

	UE_DEPRECATED(5.6, "Use IPrimitiveTransformUpdater instead")
