...
namespace EMobileAntiAliasingMethod
{
	enum Type : int
	{
		...
		TSR = AAM_TSR UMETA(DisplayName = "Temporal Super-Resolution (TSR)"),
...
class URendererSettings : public UDeveloperSettings
{
	...
	UPROPERTY(config, EditAnywhere, Category = Mobile, meta = (
		ConsoleVariable = "r.Mobile.AntiAliasing", DisplayName = "Anti-Aliasing Method",
		ToolTip = "The mobile default anti-aliasing method."))
	TEnumAsByte<EMobileAntiAliasingMethod::Type> MobileAntiAliasing;

	UPROPERTY(config, EditAnywhere, Category = Mobile, meta = (
		ConsoleVariable = "r.Mobile.FloatPrecisionMode", DisplayName = "Float Precision Mode",
	...
	*/
	UPROPERTY(config, EditAnywhere, Category = Optimizations, meta = (
		ConsoleVariable = "r.ClusteredDeferredShading.EnableForProject", DisplayName = "Support Clustered Deferred Shading",
		ToolTip = "Whether to support Clustered Deferred Shading for the project. When enabled, more shaders are compiled and the feature can be toggled on/off at runtime using r.UseClusteredDeferredShading.",
		ConfigRestartRequired = true))
		uint32 bClusteredDeferredShadingenableForProject : 1;
	...
	*/
	...
	UPROPERTY(config, EditAnywhere, Category = "Heterogeneous Volumes", meta = (
		ConsoleVariable = "r.HeterogeneousVolumes", DisplayName = "Heterogeneous Volumes (Beta)",
	...
	*/
	...
	/**
	"Enable Substrate materials."
	*/
	UPROPERTY(config, EditAnywhere, Category = Substrate, meta = (
		ConsoleVariable = "r.Substrate", DisplayName = "Substrate materials",
		ToolTip = "Enable Substrate materials.",
		ConfigRestartRequired = true))
		uint32 bEnableSubstrate : 1;

	/**
	"Enable Substrate storage format."
	*/
	UPROPERTY(config, EditAnywhere, Category = Substrate, meta = (
		ConsoleVariable = "r.Substrate.ProjectGBufferFormat",
		DisplayName = "Substrate GBuffer Format (Project)",
		ToolTip = "Select the GBuffer format used for this project.\n - Blendable GBuffer is supported on all platforms (=> 1 closure and 20 bytes per pixel maximum). \n - Adaptive GBuffer is supported on certains platforms. Unsupported platforms will fallback onto 'Blendable GBuffer' format.\nChanging this setting requires restarting the editor.",
		ConfigRestartRequired = true))
	TEnumAsByte<ESubstrateStorageFormat::Type> SubstrateGBufferFormat;

	/**
	"Substrate maximum number of closure evaluated per pixel for this project."
	*/
	UPROPERTY(config, EditAnywhere, Category = Substrate, meta = (
		ConsoleVariable = "r.Substrate.ProjectClosuresPerPixel", 
		DisplayName = "Substrate Closure Per Pixel (Project)",
		ToolTip = "Define the maximum number of closures evaluated per pixel for this project. Materials with a number of closures beyond this limit will be simplified until it fits the requirements. This only applies when Adaptive GBuffer is enabled. On platforms not supporting Adaptive GBuffer, the material will be simplified into a single closure",
		ClampMin = "1", ClampMax = "8", 
		EditCondition = "SubstrateGBufferFormat == 1",
		ConfigRestartRequired = true))
	uint32 SubstrateProjectClosuresPerPixel;

	/**
	"Substrate maximum number of closure evaluated per pixel override behavior."
	*/
	UPROPERTY(config, EditAnywhere, Category = Substrate, meta = (
		ConsoleVariable = "r.Substrate.ProjectClosuresPerPixelOverride",
		DisplayName = "Substrate Closure-Per-Pixel Override",
		ToolTip = "Select how the maximum number of closures evaluated per pixel is defined for each platform.\n - Use Platform Closure Count: Use the minimum between the platform's closure count and the project's closure count.\n - Force Closure Count: Force the project's closure count on platforms supporting Adaptive GBuffer.",
		EditCondition = "SubstrateGBufferFormat == 1",
		ConfigRestartRequired = true))
	TEnumAsByte<ESubstrateClosureConfig::Type> SubstrateClosurePerPixelOverride;

	/**
	"Substrate maximum number of bytes allocated per pixels for storing GBuffer information for this project with Adaptive GBuffer."
	*/
	UPROPERTY(config, EditAnywhere, Category = Substrate, meta = (
		ConsoleVariable = "r.Substrate.BytesPerPixel", 
		DisplayName = "Substrate Bytes Per Pixel",
		...
		ClampMin = "20", ClampMax = "256", 
		EditCondition = "SubstrateGBufferFormat == 1",
		ConfigRestartRequired = true))
	uint32 SubstrateProjectBytesPerPixel;
	...
	*/
	UPROPERTY(config, EditAnywhere, Category = Substrate, meta = (
		...
		ToolTip = "Enable Substrate opaque material rough refractions effect from top layers over layers below. (Experimental for now since this is not a path validated in production). Requires adaptive GBuffer format and at least 2 closures per pixel.",
		EditCondition = "SubstrateGBufferFormat == 1 && SubstrateProjectClosuresPerPixel > 1",
		ConfigRestartRequired = true))
	...
	*/
	...
	UPROPERTY(config, EditAnywhere, Category = Mobile, meta = (
		...
		DisplayName = "Local Light Setting (When using ForwardShading)",
	...
	UPROPERTY(config, EditAnywhere, Category = Mobile, meta = (
		...
		DisplayName = "Clustered reflections (When using ForwardShading)",
	...
	UPROPERTY(config, EditAnywhere, Category = Mobile, meta = (
		ConsoleVariable = "r.Mobile.AmbientOcclusion", DisplayName = "Ambient Occlusion (When using ForwardShading)",
	...
	UPROPERTY(config, EditAnywhere, Category = Mobile, meta = (
		ConsoleVariable = "r.Mobile.ScreenSpaceReflections", DisplayName = "Screen Space Reflections (When using Forward Shading)",
	...
	UPROPERTY(config, EditAnywhere, Category = Mobile, meta = (
		ConsoleVariable = "r.Mobile.PropagateAlpha", DisplayName = "Mobile Alpha Output",
		ToolTip = "Enable r.Mobile.PropagateAlpha to enforce alpha in scene color (overriding r.SceneColorFormat if necessary) and propagate it through the mobile renderer's post-processing chain.",
		ConfigRestartRequired = true))
	uint32 bMobileEnableAlphaChannelInPostProcessing : 1;
	...
	UPROPERTY(config, EditAnywhere, Category = Nanite, meta = (
		ConsoleVariable = "r.Nanite.Foliage", DisplayName = "Nanite Foliage (Experimental)",
		ToolTip = "Enables experimental features required to support Nanite Foliage.",
		ConfigRestartRequired = true))
	uint32 bEnableNaniteFoliage : 1;
...
private:
	...
	void SanitizeReflectionCaptureResolution();
