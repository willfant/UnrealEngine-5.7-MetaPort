...
static TAutoConsoleVariable<bool> CVarNaniteVoxelsUseSourceMeshUVDensity(
	TEXT("r.Nanite.Voxels.UseSourceMeshUVDensity"),
	1,
	TEXT("Use source mesh data when calculating UV densities for Nanite meshes that simplify into voxels."),
	ECVF_ReadOnly);
...
static bool ShouldStripNaniteFallbackMesh(const ITargetPlatform* TargetPlatform, const UStaticMesh* StaticMesh)
{
	return DoesTargetPlatformSupportNanite(TargetPlatform)
		...
		&& StaticMesh->GetNaniteSettings().GenerateFallback != ENaniteGenerateFallback::Enabled
...
bool ShouldUseSourceMeshForUVDensity(const FMeshNaniteSettings& NaniteSettings)
{
	return NaniteSettings.bEnabled &&
		NaniteSettings.ShapePreservation == ENaniteShapePreservation::Voxelize &&
		CVarNaniteVoxelsUseSourceMeshUVDensity.GetValueOnAnyThread();
}

void ShowNaniteAssembliesDisabledWarningNotification()
{
	static std::atomic<bool> bSuppressNotification = false;
	if (bSuppressNotification.load())
	{
		return;
	}
	...
	if (!IsInGameThread())
	{
		// Show the notification on the game thread only
		AsyncTask(ENamedThreads::GameThread, ShowNaniteAssembliesDisabledWarningNotification);
		return;
	}

	static const FText NaniteAssembliesWarningText = LOCTEXT("NaniteAssembliesDisabledWarning",
		"A Nanite Assembly asset was encountered while Nanite Assemblies are currently disabled. It may not appear as expected. "
		"Enable Nanite Foliage in Project Settings to enable support for Nanite Assemblies.");
	static const FText DontShowAgainMessageText = LOCTEXT("NaniteAssembliesEnabledWarningDoNotShow", "Don't show this again");
	static const FText DismissButtonText = LOCTEXT("NaniteAssembliesEnabledWarningDismiss", "Dismiss");
	static TWeakPtr<SNotificationItem> CurrentNotification;
	...
	FNotificationInfo Info(NaniteAssembliesWarningText);
	Info.Image = FAppStyle::GetBrush("Icons.WarningWithColor");
	Info.bFireAndForget = false;
	Info.CheckBoxText = DontShowAgainMessageText;
	Info.CheckBoxState = ECheckBoxState::Unchecked;
	Info.CheckBoxStateChanged = FOnCheckStateChanged::CreateLambda(
		[&](ECheckBoxState CheckboxValue)
		{
			bSuppressNotification.store(CheckboxValue == ECheckBoxState::Checked);
		}
	);
	...
	Info.ButtonDetails.Emplace(
		...
		FSimpleDelegate::CreateLambda(
			[&]()
			{
				if (TSharedPtr<SNotificationItem> Notification = CurrentNotification.Pin())
				{
...
void FStaticMeshAsyncBuildWorker::DoWork()
{
	...
	UE_LOG_CONTEXT("StaticMesh", FSoftObjectPath(StaticMesh));
...
bool FStaticMeshLODResources::IsLODInlined(const ITargetPlatform* TargetPlatform, UStaticMesh* StaticMesh, int32 LODIdx, bool bIsBelowMinLOD)
{
	...
	const int32 NumStreamedLODsOverride = StaticMesh->GetNumStreamedLODs().GetValueForPlatform(*TargetPlatform->IniPlatformName());
	...
	{
		const FStaticMeshLODGroup& LODGroupSettings = TargetPlatform->GetStaticMeshLODSettings().GetLODGroup(StaticMesh->GetLODGroup());
	...
	)
{
	...
	Params.bIsCoarseProxy				= InParentMesh->IsNaniteEnabled() && InParentMesh->GetNaniteSettings().FallbackPercentTriangles < 1.0f;
#endif
...
void FStaticMeshLODResources::InitResources(UStaticMesh* Parent, int32 LODIndex)
{
	...
	ENQUEUE_RENDER_COMMAND(NameRHIResources)(
		[this, DebugName = (Parent ? Parent->GetFName() : NAME_None)](FRHICommandListImmediate& RHICmdList)
	{
		TStringBuilder<512> StringBuilder;
		auto SetDebugName = [&RHICmdList, &StringBuilder, DebugName](FRHIBuffer* RHIBuffer, const TCHAR* Extension)
		{
			if (RHIBuffer)
			{
				StringBuilder.Reset();
				...
				RHICmdList.BindDebugLabelName(RHIBuffer, StringBuilder.ToString());
			}
		};
...
void FStaticMeshRenderData::Serialize(FArchive& Ar, UStaticMesh* Owner, bool bCooked)
{
	...
	{
		bHasNaniteFallbackMesh = (Owner->GetNaniteSettings().GenerateFallback == ENaniteGenerateFallback::Enabled || !Ar.CookingTarget()->ShouldStripNaniteFallbackMeshes());
	}
...
{
	// TODO: This check is disabled because in some cases we end up calling InitResources(...) on already initialized RenderData
	// due to an issue with PostEditUndo + async builds (UE-365996)
	...
	bForceGenerateNaniteFallbackMesh = Owner->GetNaniteSettings().GenerateFallback == ENaniteGenerateFallback::Enabled;
#endif
	...
	if (IsRayTracingAllowed() && RayTracingProxy != nullptr) // TODO: Could move most of this to FStaticMeshLODResources::InitResources
	{
		...
		ENQUEUE_RENDER_COMMAND(InitStaticMeshRayTracingGeometry)(
			[this, Owner](FRHICommandListImmediate& RHICmdList)
			{
				...
				if (RayTracingGeometryGroupHandle == INDEX_NONE) // workaround for FStaticMeshRenderData::InitResources(...) being called on already initialized RenderData due to bug with PostEditUndo + async build (UE-365996)
				{
...
void FStaticMeshRenderData::ResolveSectionInfo(UStaticMesh* Owner)
{
	...
	for (; LODIndex < MaxLODs; ++LODIndex)
	{
		...
		if (Owner->GetAutoComputeLODScreenSize())
		{
...
FMeshReductionSettings UStaticMesh::GetReductionSettings(int32 LODIndex) const
{
	...
	const FStaticMeshLODGroup& SMLODGroup = LODSettings.GetLODGroup(GetLODGroup());
	const FStaticMeshSourceModel& SrcModel = GetSourceModel(LODIndex);
...
bool UStaticMesh::GetEnableLODStreaming(const ITargetPlatform* TargetPlatform) const
{
	...
	const FStaticMeshLODGroup& LODGroupSettings = TargetPlatform->GetStaticMeshLODSettings().GetLODGroup(GetLODGroup());
...
static FString BuildStaticMeshDerivedDataKeySuffix(const ITargetPlatform* TargetPlatform, UStaticMesh* Mesh, const FStaticMeshLODGroup& LODGroup)
{
	...
	if (Mesh->IsNaniteEnabled())
	{
		...
		SerializeNaniteSettingsForDDC(Ar, Mesh->GetNaniteSettings(), Mesh->IsNaniteForceEnabled());

		if (Mesh->IsNaniteAssembly() && NaniteAssembliesSupported())
		{
			// include a hash of the references' DDC keys to invalidate the assembly when its references change
			check(Mesh->HasCachedNaniteAssemblyReferences());

			TMemoryHasher<FXxHash64Builder, FXxHash64> HashAr;
			HashAr.SetIsPersistent(true);
		
			for (UStaticMesh* Reference : Mesh->GetCachedNaniteAssemblyReferences())
			{
				if (Reference != nullptr)
				{
					const FStaticMeshLODGroup& RefLODGroup = TargetPlatform->GetStaticMeshLODSettings().GetLODGroup(Reference->GetLODGroup());
					FString RefKey = BuildStaticMeshDerivedDataKey(BuildStaticMeshDerivedDataKeySuffix(TargetPlatform, Reference, RefLODGroup));
					HashAr << RefKey;
				}
				else
				{
		...
		if (ShouldUseSourceMeshForUVDensity(Mesh->GetNaniteSettings()))
		{
			KeySuffix += TEXT("_SMUVD_");
		}

		{
			// Value of this CVar may effect adaptive tessellation
			static const TConsoleVariableData<int32>* CVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.Nanite.AdaptiveTessellation.Mode"));
			if (CVar)
			{
				const int32 Mode = CVar->GetValueOnAnyThread();
				if (Mode != 0)
				{
					// no invalidation needed for mode 0, which corresponds to behavior before CVar was introduced
					KeySuffix += FString::Printf(TEXT("_AT%d"), CVar->GetValueOnAnyThread());
				}
			}
		}
	}
	...
	if (Mesh->bSupportRayTracing && TargetPlatform->UsesRayTracing())
	{
		...
		if (CVarStaticMeshRayTracingProxies.GetValueOnAnyThread() && (Mesh->GetRayTracingProxySettings().bEnabled || ShouldGenerateRayTracingProxiesByDefault()))
		{
			TempBytes.Reset();
			FMemoryWriter Ar(TempBytes, /*bIsPersistent=*/ true);
			SerializeRayTracingProxySettingsForDDC(Ar, Mesh->GetRayTracingProxySettings());
...
void FStaticMeshRenderData::ComputeUVDensities(UStaticMesh* Mesh)
{
	...
	for (int32 LODIndex = 0; LODIndex < LODResources.Num(); ++LODIndex)
	{
		...
		for (FStaticMeshSection& SectionInfo : LODModel.Sections)
		{
			...
			bool bComputedFromSourceModel = false;
			if (LODIndex == 0 && ShouldUseSourceMeshForUVDensity(Mesh->GetNaniteSettings()))
			{
				if (FMeshDescription* MeshDescription = Mesh->GetMeshDescription(0))
				{
					const TArray<FStaticMaterial>& MeshMaterials = Mesh->GetStaticMaterials();
					if (MeshMaterials.IsValidIndex(SectionInfo.MaterialIndex))
					{
						const FName MaterialSlotName = MeshMaterials[SectionInfo.MaterialIndex].ImportedMaterialSlotName;
						FUVDensityAccumulatorSourceMesh Accumulator(*MeshDescription);
						bComputedFromSourceModel |= Accumulator.AccumulateDensitiesForMaterial(
							MaterialSlotName,
							MakeArrayView(SectionInfo.UVDensities, UE_ARRAY_COUNT(SectionInfo.UVDensities)),
							MakeArrayView(SectionInfo.Weights, UE_ARRAY_COUNT(SectionInfo.Weights)));
					}
				}

				if (Mesh->IsNaniteAssembly())
				{
					check(Mesh->HasCachedNaniteAssemblyReferences());
					const TArray<FNaniteAssemblyPart>& AssemblyParts = Mesh->GetNaniteSettings().NaniteAssemblyData.Parts;
					for (int32 PartIndex = 0; PartIndex < AssemblyParts.Num(); ++PartIndex)
					{
						UStaticMesh* PartMesh = Mesh->GetCachedNaniteAssemblyReferences()[PartIndex];
						if (PartMesh == nullptr)
						{
							continue;
						}
						...
						if (MaterialRemap.Num() > 0)
						{
							...
							{
								if (MaterialRemap[i] == SectionInfo.MaterialIndex)
								{
									PartMaterialIndices.Add(i);
								}
							}
						}
						else if (PartMaterials.IsValidIndex(SectionInfo.MaterialIndex))
						{
							PartMaterialIndices.Add(SectionInfo.MaterialIndex);
						}
						...
						for (const FNaniteAssemblyNode& Node : Mesh->GetNaniteSettings().NaniteAssemblyData.Nodes)
						{
						...
						{
							if (PartMaterials.IsValidIndex(MaterialIndexInPart))
							{
			...
			if (!bComputedFromSourceModel)
			{
				...
				for (uint32 TriangleIndex = 0; TriangleIndex < SectionInfo.NumTriangles; ++TriangleIndex)
				{
					...
					if (Area > UE_SMALL_NUMBER)
					{
						for (int32 UVIndex = 0; UVIndex < NumTexCoords; ++UVIndex)
						{
							...
							UVDensityAccs[UVIndex].PushTriangle(Area, UVArea);
						}
					}
				}
				...
				for (int32 UVIndex = 0; UVIndex < NumTexCoords; ++UVIndex)
				{
					UVDensityAccs[UVIndex].AccumulateDensity(SectionInfo.UVDensities[UVIndex], SectionInfo.Weights[UVIndex]);
				}
			}
			...
			for (int32 UVIndex = 0; UVIndex < NumTexCoords; ++UVIndex)
			{
				if (SectionInfo.Weights[UVIndex] > UE_SMALL_NUMBER)
				{
					SectionInfo.UVDensities[UVIndex] /= SectionInfo.Weights[UVIndex];
				}
			}
		}
	}
...
void FStaticMeshRenderData::Cache(const ITargetPlatform* TargetPlatform, UStaticMesh* Owner, const FStaticMeshLODSettings& LODSettings)
{
	...
	if (Owner->IsNaniteAssembly() && !NaniteAssembliesSupported())
	{
		// Pop up a notification to warn the user about assembly support
		ShowNaniteAssembliesDisabledWarningNotification();
	}
...

	{
		...
		const FStaticMeshLODGroup& LODGroup = LODSettings.GetLODGroup(Owner->GetLODGroup());
...
#endif
		UE_LOG_CONTEXT("StaticMeshKeySuffix", KeySuffix);
		...
		else
		{
			...
			if (bSaveDDC)
			{
				...
				if (bSavedToDDC)
				{
					...
					if (NaniteResources.HasStreamingData() && !IsRunningCookCommandlet())
					{
	...
	if (CVar->GetValueOnAnyThread(true) != 0 || Owner->bGenerateMeshDistanceField)
	{
		if (LODResources.IsValidIndex(0))
		{
			...
			else
			{
				...
				if (Owner->GetRenderData()->LODResources.IsValidIndex(0))
				{
				...
				{
					UE_LOGFMT(LogStaticMesh, Error, "{StaticMesh}: Failed to fetch distance field data from LOD 0 on base render data for platform {PlatformName}", 
						FSoftObjectPath(Owner), TargetPlatform->PlatformName());
				}
...
void UStaticMesh::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyName == GetLightMapResolutionName())
	{
	...
	if (!GetAutoComputeLODScreenSize()
		&& GetRenderData()
		&& PropertyName == UStaticMesh::GetAutoComputeLODScreenSizeMemberName())
	{
	...
	if (PropertyName == UStaticMesh::GetNaniteSettingsMemberName())
	{
...
void UStaticMesh::SetLODGroup(FName NewGroup, bool bRebuildImmediately, bool bAllowModify)
{
	...
	bool bResetSectionInfoMap = (GetLODGroup() != NewGroup);
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	LODGroup = NewGroup;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	if (NewGroup != NAME_None)
	{
		...
		for (int32 LODIndex = 0; LODIndex < DefaultLODCount; ++LODIndex)
		{
			...
			const bool bIsReductionActive = IsReductionActive(LODIndex);
			if (LODIndex != 0 && bIsReductionActive)
			{
				//Reset the section info map
				if (bResetSectionInfoMap)
				{
					const int32 NumberOfSections = GetSectionInfoMap().GetSectionNumber(LODIndex);
					for (int32 SectionIndex = 0; SectionIndex < NumberOfSections; ++SectionIndex)
					{
		...
		{
			SetAutoComputeLODScreenSize(true);
		}
	}
...
void UStaticMesh::GetAssetRegistryTags(FAssetRegistryTagsContext Context) const
{
	...
	Context.AddTag(FAssetRegistryTag("NaniteFallbackPercent", FString::Printf(TEXT("%.1f"), GetNaniteSettings().FallbackPercentTriangles * 100.0f), FAssetRegistryTag::TT_Numerical));
...
FStaticMeshRenderData& UStaticMesh::GetPlatformStaticMeshRenderData(UStaticMesh* Mesh, const ITargetPlatform* Platform)
{
	...
	FString PlatformDerivedDataKey = BuildStaticMeshDerivedDataKey(
		BuildStaticMeshDerivedDataKeySuffix(Platform, Mesh, PlatformLODSettings.GetLODGroup(Mesh->GetLODGroup())));
...
void UStaticMesh::CommitMeshDescription(int32 LodIndex, const FCommitMeshDescriptionParams& Params)
{
	...
	if (LodIndex == 0)
	{
		...
		if (IsNaniteAssembly())
		{
			CachedMeshDescriptionBounds.Reset(); // Nanite Assembly bounds must come from Nanite build
		}
		else if (SourceModel.GetCachedMeshDescription())
		{
...
void UStaticMesh::CacheDerivedData()
{
	...
	if (!HasCachedNaniteAssemblyReferences() &&
		ensureMsgf(bIsInGameThread, TEXT("Nanite Assembly references were not pre-cached on the game thread. Build will likely fail")))
	{
		RecacheNaniteAssemblyReferences();
	}
#endif
...
void UStaticMesh::Serialize(FArchive& Ar)
{
	...
	if (!StripFlags.IsEditorDataStripped())
	{
		...
		SetRequiresLODDistanceConversion(Ar.UEVer() < VER_UE4_STATIC_MESH_SCREEN_SIZE_LODS);
	...
	{
		if( GetNaniteSettings().FallbackRelativeError != 1.0f )
		{
			GetNaniteSettings().FallbackTarget = ENaniteFallbackTarget::RelativeError;
		}
		else if( GetNaniteSettings().FallbackPercentTriangles != 1.0f )
		{
			GetNaniteSettings().FallbackTarget = ENaniteFallbackTarget::PercentTriangles;
		}
	}
...
void UStaticMesh::BeginPostLoadInternal(FStaticMeshPostLoadContext& Context)
{
	...
	if (!Context.bIsCookedForEditor)
	{
		...
		if (GUpdateMeshLODGroupSettingsAtLoad && GetLODGroup() != NAME_None)
		{
			SetLODGroup(GetLODGroup());
		}
	...
	// Cache the assembly references, but don't sync on the compile, the async compiler will schedule it appropriately
...
void UStaticMesh::ExecutePostLoadInternal(FStaticMeshPostLoadContext& Context)
{
	...
	if (!Context.bIsCookedForEditor)
	{
		...
		if (IsNaniteAssembly())
		{
			CalculateExtendedBounds(); // in case of changes due to references
		}
	}
	...
	if (GetRequiresLODDistanceConversion())
	{
...
void UStaticMesh::ExportStaticMeshLOD(const FStaticMeshLODResources& InLODResources, FMeshDescription& OutMeshDescription) const
{
	const int32 NumWedges = InLODResources.IndexBuffer.GetNumIndices();
	const int32 NumVertexPositions = InLODResources.VertexBuffers.PositionVertexBuffer.GetNumVertices();
	const int32 NumFaces = NumWedges / 3;

	OutMeshDescription.Empty();

	if (NumVertexPositions <= 0 || InLODResources.VertexBuffers.StaticMeshVertexBuffer.GetNumVertices() <= 0)
	{
		return;
	}
	...
	const int32 NumTexCoords = InLODResources.VertexBuffers.StaticMeshVertexBuffer.GetNumTexCoords();
	VertexInstanceUVs.SetNumChannels(NumTexCoords);

	TMap<int32, FPolygonGroupID> SectionToPolygonGroupMap;

	for (int32 SectionIndex = 0; SectionIndex < InLODResources.Sections.Num(); ++SectionIndex)
	{
		const FStaticMeshSection& Section = InLODResources.Sections[SectionIndex];
		...
		if (GetStaticMaterials().IsValidIndex(Section.MaterialIndex))
		{
			PolygonGroupImportedMaterialSlotNames[CurrentPolygonGroupID] = GetStaticMaterials()[Section.MaterialIndex].ImportedMaterialSlotName;
		}
		else
		{
	...
	for (int32 VertexIndex = 0; VertexIndex < NumVertexPositions; ++VertexIndex)
	{
		...
		VertexPositions[VertexID] = InLODResources.VertexBuffers.PositionVertexBuffer.VertexPosition(VertexIndex);
	}
	...
	for (int32 TriangleIndex = 0; TriangleIndex < NumFaces; ++TriangleIndex)
	{
		...
		{
			int32 WedgeIndex = InLODResources.IndexBuffer.GetIndex(TriangleIndex * 3 + Corner);
			...
			//NTBs
			FVector TangentX = FVector4(InLODResources.VertexBuffers.StaticMeshVertexBuffer.VertexTangentX(WedgeIndex));
			FVector TangentY = FVector(InLODResources.VertexBuffers.StaticMeshVertexBuffer.VertexTangentY(WedgeIndex));
			FVector TangentZ = FVector4(InLODResources.VertexBuffers.StaticMeshVertexBuffer.VertexTangentZ(WedgeIndex));
			...
			// Vertex colors
			if (InLODResources.VertexBuffers.ColorVertexBuffer.GetNumVertices() > 0)
			{
				VertexInstanceColors[VertexInstanceID] = FLinearColor(InLODResources.VertexBuffers.ColorVertexBuffer.VertexColor(WedgeIndex));
			}
			else
			{
			...
			{
				VertexInstanceUVs.Set(VertexInstanceID, TexCoodIdx, InLODResources.VertexBuffers.StaticMeshVertexBuffer.GetVertexUV(WedgeIndex, TexCoodIdx));
			}
		}
...
bool UStaticMesh::BuildFromMeshDescriptions(const TArray<const FMeshDescription*>& MeshDescriptions, const FBuildMeshDescriptionsParams& Params)
{
	...
	const int32 NewNumLODs = FMath::Min(MeshDescriptions.Num(), MAX_STATIC_MESH_LODS);
...
bool UStaticMesh::PollAsyncPhysicsTriMeshDataCheckComplex(bool InUseAllTriData, bool bInCheckComplexCollisionMesh) const
{
#if WITH_EDITORONLY_DATA
	if(ComplexCollisionMesh && ComplexCollisionMesh != this && bInCheckComplexCollisionMesh)
	{
		// Match GetPhysicsTriMeshDataCheck recursion limits on complex mesh to avoid loops
		return ComplexCollisionMesh->PollAsyncPhysicsTriMeshDataCheckComplex(InUseAllTriData, false);
	}
#endif
	return !IsCompiling();
}
...
{
	bool bInCheckComplexCollisionMesh = true;
	return PollAsyncPhysicsTriMeshDataCheckComplex(InUseAllTriData, bInCheckComplexCollisionMesh);
}
	...
	if (const FStaticMeshRenderData* ThisRenderData = GetRenderData())
	{
		...
		for (int32 LODIndex = 0, Num = GetRenderData()->LODResources.Num(); LODIndex < Num; LODIndex++)
		{
			...
			for (int32 SectionIndex = 0; SectionIndex < LODResources.Sections.Num(); SectionIndex++)
			{
				...
				UniqueIndex.Add(MaterialIndex);
				LastIndex = (MaterialIndex < LastIndex) ? -1 : MaterialIndex;
			}
		}
		...
		if (LastIndex == -1)
		{
			...
			TArray<int32> SortedUniqueIndex = UniqueIndex.Array();
			SortedUniqueIndex.Sort(TLess<int32>());
			
			for (int32 MaterialIndex : SortedUniqueIndex)
			{
...
void UStaticMesh::GenerateLodsInPackage()
{
	...
	IMeshUtilities& MeshUtilities = FModuleManager::Get().LoadModuleChecked<IMeshUtilities>(TEXT("MeshUtilities"));
	if (MeshUtilities.GenerateStaticMeshLODs(this, LODSettings.GetLODGroup(GetLODGroup())))
	{
		// Clear LOD settings
		SetLODGroup(NAME_None, false, false);
		const auto& NewGroup = LODSettings.GetLODGroup(GetLODGroup());
...
{
	return GetNaniteSettings().bEnabled || IsNaniteForceEnabled();
}
...
{
	return IsNaniteEnabled() && GetNaniteSettings().NaniteAssemblyData.IsValid();
}

void UStaticMesh::RecacheNaniteAssemblyReferences(bool bWaitForAsyncCompile)
{
	...
	AssemblyReferenceCache.Reserve(GetNaniteSettings().NaniteAssemblyData.Parts.Num());
	for (const FNaniteAssemblyPart& PartData : GetNaniteSettings().NaniteAssemblyData.Parts)
	{
	...
	for (UStaticMesh* Ref : AssemblyReferenceCache)
	{
		if (Ref)
		{
		...
		#if WITH_EDITOR
			Ref->OnPostMeshBuild().AddUObject(this, &UStaticMesh::OnNaniteAssemblyReferenceBuilt);
		#endif
		}
	}
}
...
{
	return !IsNaniteAssembly() || AssemblyReferenceCache.Num() == GetNaniteSettings().NaniteAssemblyData.Parts.Num();
}
...
void UStaticMesh::ClearCachedNaniteAssemblyReferences()
{
	check(IsInGameThread());
	...
	for (UStaticMesh* Ref : AssemblyReferenceCache)
	{
		if (Ref)
		{
			Ref->OnPostMeshBuild().RemoveAll(this);
		}
	}
#endif
...
void UStaticMesh::OnNaniteAssemblyReferenceBuilt(UStaticMesh* Reference)
{
	if (!IsCompiling())
	{
		...
		const FStaticMeshLODGroup& PlatformLODGroup = TargetPlatform->GetStaticMeshLODSettings().GetLODGroup(GetLODGroup());
		FString CurrentDDCKey = UE::Private::StaticMesh::BuildStaticMeshDerivedDataKey(TargetPlatform, this, PlatformLODGroup);
		FStaticMeshRenderData* PlatformRenderData = GetRenderData();
