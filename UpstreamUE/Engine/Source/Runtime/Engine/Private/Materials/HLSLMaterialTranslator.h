...
#include "Runtime/Engine/Internal/MaterialCache/MaterialCacheAttribute.h"
...
protected:
	struct FMaterialVTSampleInfo
	{
		bool operator==(const FMaterialVTSampleInfo&) const = default;
		
		int32 VirtualTextureIndex = INDEX_NONE;
		ETextureMipValueMode MipValueMode = TMVM_None;
		TextureAddress AddressU = TA_Clamp;
		TextureAddress AddressV = TA_Clamp;
		...
		int32 PageTableIndex = INDEX_NONE;
		int32 PageTableUniformIndex = INDEX_NONE;
		bool bAdaptive = false;
		bool bGenerateFeedback = false;
		bool bMeshPaint = false;
		bool bMaterialCache = false;
	...
	struct FMaterialVTStackEntry
	{
		...
		uint64 PageTableHash;
		uint64 PageTableUniformHash;
		int32 CodeIndex;
	};

	/** Discovered tags during translation */
	struct FMaterialCacheTag
	{
		/** All runtime layers of this tag */
		TArray<FMaterialCacheLayer> Layers;

		/** Allocated stack index of this tag */
		int32 StackIndex = 0;
	};
	...
	/** All material tags discovered */
	TMap<FGuid, FMaterialCacheTag> MaterialCacheTags;
	...
	/** The material uses the first person custom output to interpolate between world space and first person. */
	uint32 bUsesFirstPersonInterpolation : 1;
...
public: 
	...
	TBitArray<> GetVertexInterpolatorsOffsets(FString& VertexInterpolatorsOffsetsDefinitionCode, TArray<uint32>& UsedTexCoordIndices) const;
	...
	FDerivInfo GetDerivInfo(int32 Index, bool bAllowNonFloat = false);
...
protected:
	...
	int32 AddCodeChunkInner(uint64 Hash, const TCHAR* FormattedType, const TCHAR* FormattedCode, EMaterialValueType Type, EDerivativeStatus DerivativeStatus, bool bInlined);
	...
	/** Adds an already formatted inline or referenced code chunk */
	int32 AddInternalCodeChunk(EMaterialValueType Type, const TCHAR* InternalType, const TCHAR* FormattedCode, const TRefCountPtr<FMaterialUniformExpression>& UniformExpression = nullptr);
...
protected:
	...
	int32 AddCodeChunkInner(EMaterialValueType Type, const TCHAR* FormattedType, EDerivativeStatus DerivativeStatus, bool bInlined, const TCHAR* Format, ...);
	...
	*/	
	...
	FMaterialCacheTag& FindOrAddMaterialTagStack(const FMaterialCacheTagLayout& Layout);
	...
	int32 CustomOutput(const FString& CustomOutputName, const FString& OutputTypeString, const FString& FunctionName, int32 OutputIndex, int32 OutputCode, EMaterialCustomOutputFlags Flags); // Only used internally and intentionally not exposed on FMaterialCompiler.
	...
	*/
	...
	/**
	 * Generates all material cache structures and definitions
	 */
	void GenerateMaterialCacheSource();
