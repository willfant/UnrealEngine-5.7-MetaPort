...
FAutoConsoleVariableSink CVarRayTracingInstancesEvaluateWPOSink(FConsoleCommandDelegate::CreateLambda([]()
	{
		static int32 CachedValue = CVarRayTracingInstancesEvaluateWPO.GetValueOnGameThread();

		int32 Value = CVarRayTracingInstancesEvaluateWPO.GetValueOnGameThread();
...
static TAutoConsoleVariable<int32> CVarISMInvalidateCachedNavigationBoundsOnPartialNavigationUpdates(
	TEXT("r.InstancedStaticMeshes.InvalidateCachedNavigationBoundsOnPartialNavigationUpdates"),
	1,
	TEXT("Invalidate cached navigation bounds when doing a partial navigation update"));
	...
	virtual bool Exec_Runtime(class UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
	{
		...
		else if(FParse::Command(&Cmd, TEXT("LIST ISM")))
		{
			...
			for (TObjectIterator<UInstancedStaticMeshComponent> It; It; ++It)
			{
				...
				UStaticMesh* Mesh = ISMComponent->GetStaticMesh();
				FPrimitiveMaterialPropertyDescriptor MatDesc = ISMComponent->GetUsedMaterialPropertyDesc(GetFeatureLevelShaderPlatform_Checked(FeatureLevel));
...
{
	if (IsRayTracingAllowed() && InDesc->GetStaticMesh()->bSupportRayTracing && InDesc->GetStaticMesh()->GetRenderData()->RayTracingProxy)
	{
		RayTracingProxyLODs = &InDesc->GetStaticMesh()->GetRenderData()->RayTracingProxy->LODs;
	}
...
void FInstancedStaticMeshRenderData::ReleaseResources(FSceneInterface* Scene, const UStaticMesh* StaticMesh)
{
	...
	for (int32 LODIndex = 0; LODIndex < RayTracingProxyVertexFactories.Num(); LODIndex++)
	{
		RayTracingProxyVertexFactories[LODIndex].ReleaseResource();
	}
}
...
void FInstancedStaticMeshRenderData::BindBuffersToVertexFactories(FRHICommandListBase& RHICmdList, FStaticMeshInstanceBuffer* InstanceBuffer)
{
	...
	for (int32 LODIndex = 0; LODIndex < RayTracingProxyVertexFactories.Num(); LODIndex++)
	{
		const FStaticMeshRayTracingProxyLOD* RenderData = &(*RayTracingProxyLODs)[LODIndex];

		FInstancedStaticMeshDataType InstanceData;
		FInstancedStaticMeshVertexFactory::FDataType Data;
		// Assign to the vertex factory for this LOD.
		FInstancedStaticMeshVertexFactory& VertexFactory = RayTracingProxyVertexFactories[LODIndex];
		FInstancedStaticMeshVertexFactory::InitInstancedStaticMeshVertexFactoryComponents(*RenderData->VertexBuffers, /*ColorVertexBuffer*/ nullptr, InstanceBuffer, &VertexFactory, LightMapCoordinateIndex, bRHISupportsManualVertexFetch, Data, InstanceData);
		VertexFactory.SetData(RHICmdList, Data, InstanceBuffer ? &InstanceData : nullptr);
		VertexFactory.InitResource(RHICmdList);
	}
}
...
void FInstancedStaticMeshRenderData::InitVertexFactories()
{
	...
	if (RayTracingProxyLODs)
	{
		for (int32 LODIndex = 0; LODIndex < RayTracingProxyLODs->Num(); LODIndex++)
		{
			RayTracingProxyVertexFactories.Add(new FInstancedStaticMeshVertexFactory(FeatureLevel));
		}
	}
}
...
void FInstancedStaticMeshSceneProxy::SetupProxy(const FInstancedStaticMeshSceneProxyDesc& InProxyDesc)
{
	...
	for (int32 LODIndex = 0; LODIndex < LODs.Num(); LODIndex++)
	{
		...
		for (int32 SectionIndex = 0; SectionIndex < LODInfo.Sections.Num(); SectionIndex++)
		{
			...
			bAnySegmentUsesWorldPositionOffset |= Section.Material->IsUsingWorldPositionOffset_Concurrent(GMaxRHIShaderPlatform);
		}
	}
	...
	{
		const bool bWantsRayTracingWPO = bEvaluateWorldPositionOffset && bAnySegmentUsesWorldPositionOffset && InProxyDesc.bEvaluateWorldPositionOffsetInRayTracing;
	...
	// By default, the base class (FStaticMeshSceneProxy) creates DynamicRayTracingGeometries which is not actually used
	// because FInstancedStaticMeshSceneProxy creates it's own ray tracing geometries for each simulated instance.
	// disable bCreateDynamicRayTracingGeometries to avoid that
	bCreateDynamicRayTracingGeometries = false;
...
bool FInstancedStaticMeshSceneProxy::CanSetupRayTracingInstancedMeshBatch(int32 LODIndex) const
{
	return LODIndex < InstancedRenderData.RayTracingProxyVertexFactories.Num();
}
...
void FInstancedStaticMeshSceneProxy::CreateRenderThreadResources(FRHICommandListBase& RHICmdList)
{
	...
	// currently we setup loose uniform buffer for ray tracing based on InstancedLODIndex = 0
	// since currently non of the uniforms depend on the LOD index when InstancedLODRange = 0
	RayTracingLooseUniformBuffer = CreateLooseUniformBuffer(nullptr, &UserData_AllInstances, /*InstancedLODRange*/ 0, /*InstancedLODIndex*/ 0, EUniformBufferUsage::UniformBuffer_MultiFrame);
}
...
#if RHI_RAYTRACING
	ReleaseRayTracingDynamicInstances();
#endif
}
...
void FInstancedStaticMeshSceneProxy::SetupRayTracingInstancedMeshBatch(int32 LODIndex, int32 BatchIndex, FMeshBatch& OutMeshBatch) const
{
	OutMeshBatch.VertexFactory = &InstancedRenderData.RayTracingProxyVertexFactories[LODIndex];
	...
	BatchElement0.UserIndex = 0;
	BatchElement0.PrimitiveUniformBuffer = GetUniformBuffer();
	BatchElement0.LooseParametersUniformBuffer = RayTracingLooseUniformBuffer;
	BatchElement0.bForceInstanceCulling = true; // force ISM through Generic path even for a single instance cases
...
bool FInstancedStaticMeshSceneProxy::GetRayTracingMeshElement(int32 LODIndex, int32 BatchIndex, int32 SectionIndex, uint8 InDepthPriorityGroup, FMeshBatch& OutMeshBatch) const
{
	if (CanSetupRayTracingInstancedMeshBatch(LODIndex) && FStaticMeshSceneProxy::GetRayTracingMeshElement(LODIndex, BatchIndex, SectionIndex, InDepthPriorityGroup, OutMeshBatch))
	{
		SetupRayTracingInstancedMeshBatch(LODIndex, BatchIndex, OutMeshBatch);
		return true;
	}
	return false;
}
...
void FInstancedStaticMeshSceneProxy::SetEvaluateWorldPositionOffsetInRayTracing(FRHICommandListBase& RHICmdList, bool bEvaluate)
{
#if RHI_RAYTRACING
	if (!bSupportRayTracing)
	{
	...
	bool bPrevDynamicRayTracingGeometry = bDynamicRayTracingGeometry;

	FStaticMeshSceneProxy::SetEvaluateWorldPositionOffsetInRayTracing(RHICmdList, bEvaluate);

	if (bPrevDynamicRayTracingGeometry && !bDynamicRayTracingGeometry)
	{
		ReleaseRayTracingDynamicInstances();
	}
...
void FInstancedStaticMeshSceneProxy::GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector)
{
	...
	if (!bDynamicRayTracingInstances && !bDynamicRayTracingGeometry)
	{
		return;
	}
	...
	checkf(!InstanceSceneDataBuffers->IsInstanceDataGPUOnly(), TEXT("FInstancedStaticMeshSceneProxy does not support dynamic ray tracing geometry when IsInstanceDataGPUOnly()."));
	...
	TArray<FMeshBatch> RayTracingMeshBatches;

	{
		int32 SectionCount = (*InstancedRenderData.RayTracingProxyLODs)[LODIndex].Sections->Num();
		...
		for (int32 SectionIdx = 0; SectionIdx < SectionCount; ++SectionIdx)
		{
			...
			if (RenderData->RayTracingProxy->bUsingRenderingLODs)
			{
				...
				bResult = GetMeshElement(LODIndex, 0, SectionIdx, 0, false, false, MeshBatch);
			}
			else
			{
				// otherwise initialize MeshBatch using ray tracing proxy VB/IB/Section data
				bResult = GetRayTracingMeshElement(LODIndex, 0, SectionIdx, 0, MeshBatch);
			}
			...
			if (bResult)
			{
				if (RenderData->RayTracingProxy->bUsingRenderingLODs)
				{
					check(MeshBatch.VertexFactory == &InstancedRenderData.VertexFactories[LODIndex]);
				}
				else
				{
					check(MeshBatch.VertexFactory == &InstancedRenderData.RayTracingProxyVertexFactories[LODIndex]);
				}

				MeshBatch.ReverseCulling = bReverseCulling;

				RayTracingMeshBatches.Add(MeshBatch);
			}
		}
	}
	...
	TArray<FVector, TInlineAllocator<2, SceneRenderingAllocator>> ViewPositions;
	ViewPositions.Reserve(Views.Num());
	
	for (const FSceneView* View : Views)
	{
		ViewPositions.Add(View->ViewLocation);
	}

	auto GetDistanceToInstance = [&ViewPositions, InstanceSceneDataBuffers](int32 InstanceIndex, float& OutInstanceRadius, float& OutDistanceToInstanceCenter, float& OutDistanceToInstanceStart)
	{
		const FBoxSphereBounds& InstanceWorldBounds = InstanceSceneDataBuffers->GetInstanceWorldBounds(InstanceIndex);

		OutDistanceToInstanceCenter = std::numeric_limits<float>::infinity();

		for (const FVector& ViewPosition : ViewPositions)
		{
			const FVector VToInstanceCenter = ViewPosition - InstanceWorldBounds.Origin;

			OutDistanceToInstanceCenter = FMath::Min(OutDistanceToInstanceCenter, float(VToInstanceCenter.Size()));
		}
	...
	if (bDynamicRayTracingGeometry && VisibleInstances.Num() > 0)
	{
	...
	FRayTracingInstance StaticRayTracingInstance;
	StaticRayTracingInstance.Geometry = RayTracingGeometry;
	StaticRayTracingInstance.Materials = RayTracingMeshBatches;
	...
	StaticRayTracingInstance.PrimitiveInstanceIndices.Reserve(InstanceCount);
	...
	for (FVisibleInstance VisibleInstance : VisibleInstances)
	{
		...
		if (bDynamicRayTracingGeometry && InstancedRenderData.RayTracingProxyVertexFactories[LODIndex].GetType()->SupportsRayTracingDynamicGeometry())
		{
			...
			if (ActiveInstances[DynamicInstanceIdx] == INDEX_NONE)
			{
				...
				FRayTracingDynamicInstanceData& DynamicData = RayTracingDynamicInstanceData[DynamicInstanceIdx];

				ActiveInstances[DynamicInstanceIdx] = RayTracingInstances.Num();
				FRayTracingInstance& RayTracingInstance = RayTracingInstances.AddDefaulted_GetRef();
				RayTracingInstance.Geometry = &DynamicData.Geometry;
				RayTracingInstance.Materials = RayTracingMeshBatches;
				...
				Collector.AddRayTracingGeometryUpdate(
					...
					FRayTracingDynamicGeometryUpdateParams
					{
						...
						DynamicData.Geometry.Initializer.TotalPrimitiveCount,
						&DynamicData.Geometry,
						nullptr,
						true,
						InstanceIndex,
		...
		{
			StaticRayTracingInstance.PrimitiveInstanceIndices.Add(InstanceIndex);
		}
	}

	if (StaticRayTracingInstance.PrimitiveInstanceIndices.Num() > 0)
	{
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			...
			Collector.AddRayTracingInstance(ViewIndex, StaticRayTracingInstance);
		}
	}

	for (const FRayTracingInstance& Instance : RayTracingInstances)
	{
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			if ((VisibilityMap & (1 << ViewIndex)) == 0)
...
{
	if (NumDynamicInstances == RayTracingDynamicInstanceData.Num() && LODIndex == CachedRayTracingLODIndex)
	{
		return;
	}
	...
	for (FRayTracingDynamicInstanceData& DynamicData : RayTracingDynamicInstanceData)
	{
		DynamicData.Geometry.ReleaseResource();
		DynamicData.VertexBuffer.Release();
	}

	// clear and resize
	RayTracingDynamicInstanceData.Empty(NumDynamicInstances);
	...
	{
		FRayTracingDynamicInstanceData& DynamicData = RayTracingDynamicInstanceData.AddDefaulted_GetRef();
		...
		DynamicData.Geometry.SetInitializer(MoveTemp(Initializer));
		DynamicData.Geometry.InitResource(RHICmdList);
	}
}

void FInstancedStaticMeshSceneProxy::ReleaseRayTracingDynamicInstances()
{
	for (auto& DynamicRayTracingItem : RayTracingDynamicInstanceData)
	{
		DynamicRayTracingItem.Geometry.ReleaseResource();
		DynamicRayTracingItem.VertexBuffer.Release();
	}

	RayTracingDynamicInstanceData.Empty();
}

void FInstancedStaticMeshSceneProxy::GetRayTracingWPOConfig(bool& bOutHasRayTracingRepresentation, bool& bOutDynamicRayTracingGeometry) const
{
	FStaticMeshSceneProxy::GetRayTracingWPOConfig(bOutHasRayTracingRepresentation, bOutDynamicRayTracingGeometry);

	const int32 WPOEvalMode = CVarRayTracingInstancesEvaluateWPO.GetValueOnAnyThread();

	bOutDynamicRayTracingGeometry = WPOEvalMode < 0 ? bOutDynamicRayTracingGeometry : WPOEvalMode != 0;
	bOutDynamicRayTracingGeometry &= bAnySegmentUsesWorldPositionOffset;

	const FInstanceSceneDataBuffers* InstanceSceneDataBuffers = GetInstanceSceneDataBuffers();

	if (InstanceSceneDataBuffers->IsInstanceDataGPUOnly())
	{
		UE_LOG(LogStaticMesh, Warning, TEXT("Dynamic ray tracing geometry not currently supported for GPU instanced static meshes (%s). Falling back to static ray tracing geometry."), *StaticMesh->GetPathName());

		// TODO: Update FInstancedStaticMeshSceneProxy to make it compatible with GPU Scene when using WPO materials.
		bOutDynamicRayTracingGeometry = false;
	}
}
...
void FInstancedStaticMeshSceneProxy::GetPreSkinnedLocalBounds(FBoxSphereBounds& OutBounds) const
{
...
void UInstancedStaticMeshComponent::ComputeHLODHash(FHLODHashBuilder& HashBuilder) const
{
	...
	HashBuilder.HashField(PerInstanceSMData, GET_MEMBER_NAME_CHECKED(UInstancedStaticMeshComponent, PerInstanceSMData));
	HashBuilder.HashField(PerInstanceSMCustomData, GET_MEMBER_NAME_CHECKED(UInstancedStaticMeshComponent, PerInstanceSMCustomData));
	HashBuilder.HashField(InstancingRandomSeed, GET_MEMBER_NAME_CHECKED(UInstancedStaticMeshComponent, InstancingRandomSeed));
	HashBuilder.HashField(AdditionalRandomSeeds, GET_MEMBER_NAME_CHECKED(UInstancedStaticMeshComponent, AdditionalRandomSeeds));
}
#endif
...
void UInstancedStaticMeshComponent::SendRenderInstanceData_Concurrent()
{
	...
	if (bDetailModeAllowsRendering && (ShouldRender() || bCastHiddenShadow || bAffectIndirectLightingWhileHidden || bRayTracingFarField))
	{
		if (SceneProxy != nullptr)
		{
			...
			BuildComponentInstanceData(SceneProxy->GetScene().GetShaderPlatform(), ComponentData);
...
FPrimitiveSceneProxy* UInstancedStaticMeshComponent::CreateSceneProxy()
{
	...
	{
		FInstanceUpdateComponentDesc ComponentData;
		BuildComponentInstanceData(PrimitiveSceneProxy->GetScene().GetShaderPlatform(), ComponentData);
...
{
	BuildComponentInstanceData(GetFeatureLevelShaderPlatform_Checked(FeatureLevel), OutData);
}

void UInstancedStaticMeshComponent::BuildComponentInstanceData(EShaderPlatform InShaderPlatform, FInstanceUpdateComponentDesc& OutData)
{
	LLM_SCOPE(ELLMTag::InstancedMesh);
	QUICK_SCOPE_CYCLE_COUNTER(STAT_UInstancedStaticMeshComponent_BuildRenderData);

	OutData.PrimitiveMaterialDesc = GetUsedMaterialPropertyDesc(InShaderPlatform);
...
void UInstancedStaticMeshComponent::InitInstanceBody(int32 InstanceIdx, FBodyInstance* InstanceBodyInstance, FBodyInstance* ReferenceBodyInstance, bool bRuntimeBodyInstance, UBodySetup* BodySetup)
{
	if (!GetStaticMesh())
	{
		UE_LOG(LogStaticMesh, Warning, TEXT("Unable to create a body instance for %s in Actor %s. No StaticMesh set."), *GetName(), GetOwner() ? *GetOwner()->GetName() : TEXT("?"));
		return;
	}
	...
	if(bRuntimeBodyInstance)
	{
		InstanceBodyInstance->CopyRuntimeBodyInstancePropertiesFrom(ReferenceBodyInstance);
	}
	else
	{
		InstanceBodyInstance->CopyBodyInstancePropertiesFrom(ReferenceBodyInstance);
	}
...
{
	InitInstanceBody(InstanceIdx, InstanceBodyInstance, &BodyInstance, false, GetBodySetup());
}
...
// This function CalcAndCacheNavigationBounds is set to be deprecated in v5.8, but because its original disuse is part of a hotfix, the header file isn't available to be modified yet.
...
void UInstancedStaticMeshComponent::SerializeRenderData(FArchive& Ar)
{
	...
	else if (Ar.IsSaving())
	{
		...
		if (GetStaticMesh() != nullptr)
		{
			...
			BuildComponentInstanceData(GMaxRHIShaderPlatform, ComponentData);
		}
...
void UInstancedStaticMeshComponent::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
	...
	if (!IsTemplate())
	{
		const bool bUseRandomSeedValue = !ObjectSaveContext.IsCooking();
		InitializeInstancingRandomSeed(bUseRandomSeedValue);
	}
}
...
int32 UInstancedStaticMeshComponent::AddInstanceInternal(int32 InstanceIndex, FInstancedStaticMeshInstanceData* InNewInstanceData, const FTransform& InstanceTransform, bool bWorldSpace)
{
	...
	{
		// Copy in the current transform - it is somewhat redundant should probably add tracking instead to ensure it gets set, or pass in a struct that can initialize the whole instance 
...
{
	if (InstanceBodies.IsValidIndex(InstanceBodyIndex))
	{
		FBodyInstance* Body = InstanceBodies[InstanceBodyIndex];
		if (Body)
		{
			const FTransform InstanceTM = FTransform(PerInstanceSMData[InstanceBodyIndex].Transform) * GetComponentTransform();

			FBodyInstance* Instance = new FBodyInstance;

			InitInstanceBody(InstanceBodyIndex, Instance, Body, true, Body->GetBodySetup());
...
void UInstancedStaticMeshComponent::OnPostLoadPerInstanceData()
{
	...
	if (AActor* Owner = GetOwner())
	{
		...
		UWorld* OwnerWorld = OwnerLevel ? OwnerLevel->OwningWorld.Get() : nullptr;
...
void UInstancedStaticMeshComponent::PartialNavigationUpdates(const TConstArrayView<FTransform> InstanceTransforms)
{
	...
	if (InstanceBounds.IsValid)
	{
		if (CVarISMInvalidateCachedNavigationBoundsOnPartialNavigationUpdates.GetValueOnAnyThread() != 0)
		{
			// Make sure to invalidate cached navigation bounds that might have been computed from a different number of instances.
			CachedBounds[static_cast<int32>(EBoundsCacheType::Navigation)].Invalidate();
		}
...
{
	return GetOrCalcBounds(GetComponentTransform(), EBoundsCacheType::Navigation).GetBox();
}
...
void UInstancedStaticMeshComponent::InitializeInstancingRandomSeed(const bool bAllowRandomSeedValue)
{
	auto GenerateSeedFromObjectPath = [this]() -> int32
	{
		TStringBuilder<256> ResultBuilder;
		GetPathName(nullptr, ResultBuilder);
		int32 RandomSeed = (int32)CityHash32((const char*)ResultBuilder.GetData(), ResultBuilder.Len() * sizeof(decltype(ResultBuilder)::ElementType));
		if (RandomSeed == 0)
		{
			// If the resulting hash is still zero (unlikely, but possible), we fall back to a fixed default value.
			RandomSeed = 42;
		}
	...
	if (bAllowRandomSeedValue)
	{
		// This code path is assumed to be used when saving.
		// Make sure to generate a truly random seed which is not based on the object path.
		int32 SeedFromPath = GenerateSeedFromObjectPath();
		while (InstancingRandomSeed == 0 || InstancingRandomSeed == SeedFromPath)
		{
	...
	{
		// To maintain determinism, the seed is generated from the component's path name.
		if (InstancingRandomSeed == 0)
		{
			InstancingRandomSeed = GenerateSeedFromObjectPath();
		}
	}	
}
